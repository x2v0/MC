...
public:
   ...
   virtual void ScorePoint(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
                           const geomVector3D& p0)
   {
      etotal_[iThread] += edep;
   }

   virtual void ScoreLine(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
   ...
   {
      return attached_module_;
   }

   void setTransport(mcTransport* t)
   {
      transport_ = t;
   ...
   virtual void dumpVRML(ostream&) const; // Вывод содержимого перемещен не в оператор, а в функцию,
