...
// Класс транспорта в наборе вложенных цилиндров, 
// воспроизводящих гребенку для размывания спектра заряженных частиц
class mcTransportCylinderStack : public mcTransport
{
public:
   mcTransportCylinderStack();

   mcTransportCylinderStack(const geomVector3D& orgn, const geomVector3D& z, const geomVector3D& x, int nc, double dr,
                            double ds, double h);
...
protected: // Объект описывается как цилиндрические кольца постоянной толщины,
   // следующие с постоянным шагом.
   // Количество колец
   int nc_;    // Толщина кольца
   double dr_; // Шаг следования колец по радиусу
