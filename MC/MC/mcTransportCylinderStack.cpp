...
mcTransportCylinderStack::mcTransportCylinderStack(void) : mcTransport()
{
   setGeometry(0, 0, 0, 0);
}

mcTransportCylinderStack::mcTransportCylinderStack(const geomVector3D& orgn, const geomVector3D& z,
                                                   const geomVector3D& x, int nc, double dr, double ds,
...
{
   p.exitSurface_ = mcParticle::temb_shit_t::External; // Удаление от секущих плоскостей
