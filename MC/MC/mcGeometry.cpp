...
double mcGeometry::getDistanceToInfiniteCylinderInside(const geomVector3D& p, const geomVector3D& v, double r)
{
   ...
   if (a < DBL_EPSILON)
      return DBL_MAX; // Параллельно оси
...
{
   // Цилиндр
   double cd = getDistanceToInfiniteCylinderInside(p, v, r); // Плоскости
   double vz = v.z();
...
double mcGeometry::getDistanceToCylinderOutside(const geomVector3D& p, const geomVector3D& v, double r, double h)
{
   ...
   double cd = getDistanceToInfiniteCylinderOutside(p, v, r);
   if (cd == DBL_MAX)
      return DBL_MAX;
   if (z <= 0) {
      if (fabs(vz) <= DBL_EPSILON)
         return DBL_MAX;
      double pd = -z / vz;
      geomVector3D pp = p + (v * pd);
      if (pp.sqLengthXY() < R2)
...
double mcGeometry::getDistanceToPrismInside(const geomVector3D& p, const geomVector3D& v, double ax, double ay,
                                            double h)
{
   double dx = (v.x() < 0)
                  ? fabs((p.x() + 0.5 * ax) / v.x())
...
double mcGeometry::getDistanceToPrismOutside(const geomVector3D& p, const geomVector3D& v, double ax, double ay,
                                             double h)
{
...
double mcGeometry::getDistanceToConeInside(const geomVector3D& p, const geomVector3D& v, double r, double f)
{
   ...
   double vx = v.x(), vy = v.y(), vz = v.z(); // Вариант 1. Частица вылетает из конуса в бесконечность.
   double rr = sqrt(r * r + f * f);
   if (vz < 0 && -vz >= f / rr - FLT_EPSILON)
      return DBL_MAX; // Параметры квадратного уравнения
   double t = vz * r / f;
   double a = vx * vx + vy * vy - t * t;
   t = 1 - z / f;
   double b = x * vx + y * vy + r * r * t * vz / f;
   double c = x * x + y * y - r * r * t * t;
   double det = b * b - a * c; // Вариант 2. a = 0 сооствествует движению вдоль поверхности.
   ...
   if (det <= 0)
      return DBL_MAX; // Если все правильно и частица находится внутри, то единственное правильное
...
double mcGeometry::getDistanceToConeOutside(const geomVector3D& p, const geomVector3D& v, double r, double f)
{
   ...
   geomVector3D nn(p.x() * s, p.y() * s, -f);
   // нормаль к поверхности конуса в плоскости сечения, проходящей через ось Z и точку частицы
   ...
   double b = x * vx + y * vy + r * r * t * vz / f;
   double c = x * x + y * y - r * r * t * t;
   double det = b * b - a * c; // Пересечений может не быть вообще
   if (det <= 0)
      return DBL_MAX;
   double cd;
   if (fabs(a) <= FLT_EPSILON)
...
double mcGeometry::getDistanceToConeSlabInside(const geomVector3D& p, const geomVector3D& v, double z1, double z2,
                                               double r1, double r2)
{
   double dd = 0;
   if (r2 == r1)
      dd = getDistanceToInfiniteCylinderInside(p, v, r1);
   else {
      double x = p.x(), y = p.y(), z = p.z();
      double vx = v.x(), vy = v.y(), vz = v.z();
...
double mcGeometry::getDistanceToConeSlabOutside(const geomVector3D& p, const geomVector3D& v, double z1, double z2,
                                                double r1, double r2)
{
   double dd = 0;
   if (r2 == r1)
      dd = getDistanceToInfiniteCylinderOutside(p, v, r1);
   else {
      double x = p.x(), y = p.y(), z = p.z();
      double vx = v.x(), vy = v.y(), vz = v.z();
      double f = (r2 - r1) / (z2 - z1);
...
double mcGeometry::getDistanceToRectanglePipeInside(const geomVector3D& p, const geomVector3D& v, double x1, double x2,
                                                    double y1, double y2)
{
   double dx1 = p.x() - x1;
...
double mcGeometry::getDistanceToRectanglePipeOutside(const geomVector3D& p, const geomVector3D& v, double x1, double x2,
                                                     double y1, double y2)
{
   // Упрощаем задачу так, чтобы частица двигалась по всем координатам 
   // в положительном направлении и стартовая координата задавалась
...
double mcGeometry::getDistanceToRectangleConeInside(const geomVector3D& p, const geomVector3D& v, double x1, double x2,
                                                    double cosx, double sinx, double y1, double y2, double cosy,
                                                    double siny, double z)
{
   double px = p.x(), py = p.y(), pz = p.z();
   double vx = v.x(), vy = v.y(), vz = v.z(); // Скалярное произведение вектора скорости и нормали к грани
...
double mcGeometry::getDistanceToRectangleConeOutside(const geomVector3D& p, const geomVector3D& v, double x1, double x2,
                                                     double cosx, double sinx, double y1, double y2, double cosy,
                                                     double siny, double z)
{
   double px = p.x(), py = p.y(), pz = p.z();
...
double mcGeometry::TrLenInVoxel(double x1, double y1, double z1, double x2, double y2, double z2, double xv1,
                                double yv1, double zv1, double xv2, double yv2, double zv2)
{
   double f; // Make partical direction positive along Z-axis.
   if (z2 < z1) {
      f = x1;
      x1 = x2;
      x2 = f;
...
double mcGeometry::getDistanceToSphereInside(const geomVector3D& p, const geomVector3D& v, double r)
{
   ...
   double det = b * b - c;
   if (det < 0)
      det = 0; // eliminate surface vicinity problem
...
{
   double b = p * v; // if moves out of sphere
   if (b >= 0)
      return DBL_MAX;
   double c = p * p - r * r;
   double det = b * b - c;
   if (det <= 0)      // no intersection
      return DBL_MAX; // Only two solutions can exist. Smallest distance is what we are looking for.
   // If it is negative, then we move out of sphere.
   double dist = -b - sqrt(det);
   return dist >= 0 ? dist : -dist;
}

double mcGeometry::getDistanceToConvexPolygonCircleInside(const geomVector3D& p, const geomVector3D& v,
                                                          const std::vector<double>& pz, const std::vector<double>& pr)
{
   ...
   double z = p.z();
   for (i = 1; i < pz.size(); i++)
      if (z <= pz[i] + DBL_EPSILON)
         break;
   double z1 = pz[i - 1], z2 = pz[i];
   double r1 = pr[i - 1], r2 = pr[i];
   double vz = v.z(); // Движение параллельно плоскости XY
   if (vz == 0) {
...
double mcGeometry::getDistanceToConvexPolygonCircleOutside(const geomVector3D& p, const geomVector3D& v,
                                                           const std::vector<double>& pz, const std::vector<double>& pr)
{
