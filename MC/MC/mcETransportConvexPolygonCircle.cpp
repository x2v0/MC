...
mcETransportConvexPolygonCircle::mcETransportConvexPolygonCircle(const geomVector3D& orgn, const geomVector3D& z,
                                                                 const geomVector3D& x, std::vector<double>& pz,
                                                                 std::vector<double>& pr) : mcTransport(orgn, z, x),
                                                                                            pz_(pz), pr_(pr) {}
...
void mcETransportConvexPolygonCircle::dump(ostream& os) const
{
   ...
   os << "Polygon:\t" << endl;
   unsigned i;
   for (i = 0; i < pz_.size(); i++)
      os << pz_[i] << "\t";
   os << endl;
   for (i = 0; i < pr_.size(); i++)
      os << pr_[i] << "\t";
   os << endl << endl;
}
