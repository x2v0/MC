...
{
public:                                             // Types
   enum open_mode_e { NOT_OPEN, READING, WRITING }; // Ctor
   mcPhaseSpaceIO();                                // Dtor
   virtual ~mcPhaseSpaceIO();                       // Accessors
   unsigned int numParticles() const
   {
      return preamble_.n_particles;
   }

   unsigned int latch() const
   {
      return latch_;
   }
   ...
   unsigned int birthReg() const; // Modifiers
...
private: // Types
   ...
   FILE* file;
   preamble_t preamble_; // Keep latch of the last read particle
   unsigned int latch_;
};
