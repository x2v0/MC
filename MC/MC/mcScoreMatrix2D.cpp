...
mcScoreMatrix2D::mcScoreMatrix2D(const char* module_name, int nThreads, int nx, int nz, const geomVector3D& v1,
                                 const geomVector3D& v2) : mcScore(module_name, nThreads), m_nx(nx), m_nz(nz)
{
   if (v1.y() > v2.y()) {
      m_ymin = v2.y();
      m_ymax = v1.y();
...
void mcScoreMatrix2D::ScorePoint(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
...
void mcScoreMatrix2D::ScoreLine(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
                                const geomVector3D& p0, const geomVector3D& p1)
{
   ...
   if (x1 < m_xmin && x2 < m_xmin)
      return;
   if (x1 > m_xmax && x2 > m_xmax)
      return;
   if (y1 < m_ymin && y2 < m_ymin)
      return;
   if (y1 > m_ymax && y2 > m_ymax)
      return;
   if (z1 < m_zmin && z2 < m_zmin)
      return;
   if (z1 > m_zmax && z2 > m_zmax)
      return; // Index limits
   ...
   int iz2 = (z2 < m_zmin) ? -1 : int((z2 - m_zmin) / m_zstep); // When path is completely within one voxel
   if (ix1 == ix2 && iz1 == iz2 && y1 >= m_ymin && y1 <= m_ymax && y2 >= m_ymin && y2 <= m_ymax) {
...
void mcScoreMatrix2D::dumpStatistic(ostream& os) const
{
   ...
   os << nx << '\t' << nz << '\t' << xmin << '\t' << ymin << '\t' << zmin << '\t' << xmax << '\t' << ymax << '\t' <<
      zmax << endl << endl;
   for (ix = 0; ix < nx; ix++)
...
void mcScoreMatrix2D::dumpVRML(ostream& os) const
{
   ...
   os << "      point [" << endl; // √рани XY (четные точки нам Zmin, нечетные - Zmax)
   unsigned ncount = 0;
   for (j = 0; j < nj; j++) {
