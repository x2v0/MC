...
bool mcPhysicsPositron::Discarge(mcParticle* p, const mcMedium& med, double& edep) const
{
   if (p->ke <= ((const mcMediumXE&)med).transCutoff_elec || p->ke <= p->transport_->transCutoff_elec) {
      edep = p->ke;
      AnnihilateAtRest(p); // Частицу не уничтожаем, хотя говорим, что да.
...
double mcPhysicsPositron::TakeOneStep(mcParticle* p, const mcMedium& med, double& step) const
{
   ...
   if (iLogKE < 0)
      iLogKE = 0; // вблизи энергии поглощения возможна проблема индексов
...
double mcPhysicsPositron::DoInterruction(mcParticle* p, const mcMedium* med) const
{
   ...
   double logKE = log(p->ke);
   int iLogKE = (int)(m->iLogKE0_elec + logKE * m->iLogKE1_elec);
   bool bremsAllowed = p->ke > m->eventCutoff_phot; // Use random numbers and tabulated branching ratios to
...
void mcPhysicsPositron::BhabhaScatter(mcParticle* p, const mcMediumXE* med)
{
   ...
   mcRng& rng = p->thread_->rng(); // Get the energies of the positron and negatron
   ...
   mcParticle* pNextParticle = DuplicateParticle(p); // Choose an arbitrary azimuthal angle
...
void mcPhysicsPositron::GetBhabhaEnergies(mcRng& rng, const mcMediumXE* pMedium, double ke_in, double* ke_posi,
                                          double* ke_nega)
{
   ...
   double b1 = 2.0 - ySquared; // Loop until a value of r (between r0 and 1) is accepted
   do {
      r = r0 / (1.0 - oneMinusR0 * rng.rnd());
   } while (rng.rnd() > (1.0 - betaSquared * r * (b1 - r * (b2 - r * (b3 - r * b4))))); // Divide up the energy
   r = MAX(0, r);
   (*ke_nega) = r * ke_in;
...
void mcPhysicsPositron::AnnihilateInFlight(mcParticle* p)
{
   ...
   mcRng& rng = p->thread_->rng(); // Get the energies of the two photons
   double e_phot1, e_phot2;
   GetAnnihilationEnergies(rng, ke_in, p_in, &e_phot1, &e_phot2); // Choose an arbitrary azimuthal angle
   double cosPhi, sinPhi;
...
void mcPhysicsPositron::GetAnnihilationEnergies(mcRng& rng, double ke_in, double p_in, double* e_phot1, double* e_phot2)
{
...
{
   mcRng& rng = p->thread_->rng(); // Change the current particle to an appropriate photon
   p->t = MCP_PHOTON;
   p->q = 0;
   p->ke = EMASS;
   p->mfps = 0;
   GoInRandomDirection(rng.rnd(), rng.rnd(), p->u); // Create a second photon moving in the opposite direction:
