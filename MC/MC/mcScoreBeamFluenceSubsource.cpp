...
mcScoreBeamFluenceSubsource::mcScoreBeamFluenceSubsource(const char* name, int nThreads, enum mc_particle_t ptype,
                                                         double zmin, double zmax, double focus, int nr, double rmax,
                                                         int ne, double emax, int na, double amax) : ptype_(ptype),
                                                                                                     zmin_(zmin),
                                                                                                     zmax_(zmax),
                                                                                                     nThreads_(
                                                                                                        nThreads),
...
double mcScoreBeamFluenceSubsource::Spectrum(int ir, int ie) const
{
   double f = 0;
   if (ir >= 0 && ir < nr_) {
      for (int i = 0; i < nThreads_; i++) //f += spectra_[i][ir*ne_ + ie];
         f += Spectrum(i, ir, ie);
   }
   return f;
}
