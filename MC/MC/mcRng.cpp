...
mcRng::~mcRng(void) {} // Initializes the RANMAR random number generator (see below). The first
...
void mcRng::init(int ijSeed, int klSeed)
{
   ...
   decrement_ = 362436.0 / 16777216.0; // Use the seeds to compute the initial values of i, j, k, l:
   int i = ((ijSeed / 177) % 177) + 2;
   int j = (ijSeed % 177) + 2;
   int k = ((klSeed / 169) % 178) + 1;
   int l = klSeed % 169; // Compute 97 generator values to 24 binary digits (bits) each:
   for (int iGen = 0; iGen < 97; iGen++) {
      genArray_[iGen] = 0.0;
      double powerOfTwo = 1.0;
      for (int iBit = 0; iBit < 24; iBit++) {
         // Set the decimal number represented by the bit:
         powerOfTwo *= 0.5; // By adding or not adding the decimal number, set the bit to one or zero:
         int m = (((i * j) % 179) * k) % 179;
         i = j;
         j = k;
         k = m;
...
double mcRng::rnd()
{
   ...
   const double incrementOfDecrement = 16777213.0 / 16777216.0; // Compute a new generator:
   double gen = *pIGen_ - *pJGen_;
   if (gen < 0.0)
      gen += 1.0; // Recompute the decrement:
   decrement_ -= decrementOfDecrement;
   if (decrement_ < 0.0)
      decrement_ += incrementOfDecrement; // Decrement the generator to obtain the random number:
   double rnd = gen - decrement_;
   if (rnd < 0.0)
      rnd += 1.0; // Save the generator and move the pointers for use during
