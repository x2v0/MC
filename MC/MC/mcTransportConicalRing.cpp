...
mcTransportConicalRing::mcTransportConicalRing(void) : mcTransport()
{
   setGeometry(0, 0, 0, 0);
}

mcTransportConicalRing::mcTransportConicalRing(const geomVector3D& orgn, const geomVector3D& z, const geomVector3D& x,
...
double mcTransportConicalRing::getDistanceOutside(mcParticle& p) const
{
   ...
   double dd = rr <= r0_
                  ...
                  : rr >= r1_
                  ? mcGeometry::getDistanceToConeOutside(p.p, p.u, r1_, f_)
                  : DBL_MAX;
   if (dd == DBL_MAX)
      return DBL_MAX;
   cd += dd;
