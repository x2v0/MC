...
mcScoreMatrixRZ::mcScoreMatrixRZ(const char* module_name, int nThreads, int nr, int nz, double rmax, double zmin,
                                 double zmax) : mcScore(module_name, nThreads)
{
   m_nr = nr;
   m_nz = nz;
   m_rmax = rmax;
...
void mcScoreMatrixRZ::ScorePoint(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
...
{
   double eddens = edep / (p1 - p0).length(); // —ортируем точки так, чтобы первой была точка с меньшим Z.
   bool doChange = p0.z() > p1.z();
   geomVector3D vz1(doChange ? p1 : p0);
   geomVector3D vz2(doChange ? p0 : p1);
   geomVector3D v = vz2 - vz1;
   v.normalize(); // ¬ектор направлени€, длина которого по Z равна 1.
   bool isOrto = fabs(v.z()) <= DBL_EPSILON;
   geomVector3D uz = isOrto ? geomVector3D() : v * (1. / v.z()); // ќбрезаем трек плоскост€ми торцов цилиндра.
   ...
   while ((vz2 - vz1) * v > DBL_EPSILON) {
      double r = ir * m_rstep;
      double d;
      bool toCenter;             // ќпредел€ем направление движени€ к центру или от него
      ...
      geomVector3D vz12 = vz1 + (v * d); // ѕровер€ем не прошли ли мы дальнюю точку глобального трека.
      if ((vz12 - vz2) * v > 0)
         vz12 = vz2; // ѕеребираем слои по Z
      ...
      while ((vz12 - pz1) * v > DBL_EPSILON) {
         ...
         if ((pz2 - vz12) * v > 0)
            pz2 = vz12; // ¬ычисл€ем индекс кольца более надежным способом.
...
void mcScoreMatrixRZ::dumpVRML(ostream& os) const
{
   ...
   os << "      point [" << endl;
   for (iz = 0; iz <= m_nz; iz++) {
      double z = m_zmin + iz * m_zstep; //  онцентрические круги
      for (ir = 1; ir <= m_nr; ir++) {
         double r = m_rstep * ir;
...
void mcScoreMatrixRZ::dumpStatistic(ostream& os) const
{
   ...
   os << nr << '\t' << nz << '\t' << rmax << '\t' << zmin << '\t' << zmax << endl << endl; //for( iz=0; iz < nz; iz++ )
