...
mcTransportCone::mcTransportCone() : mcTransport()
{
   setGeometry(0, 0);
}

mcTransportCone::mcTransportCone(const geomVector3D& orgn, const geomVector3D& z, const geomVector3D& x, double r,
                                 double h) : mcTransport(orgn, z, x)
{
   setGeometry(r, h);
}
...
double mcTransportCone::getDistanceInside(mcParticle& p) const
{
   ...
   if (det < 0) // при нулевом детерминанте попадаем точтно в вершину конуса
      return 0; // В этом месте b не может быть нулевым, так как это означало бы 
...
double mcTransportCone::getDistanceOutside(mcParticle& p) const
{
   ...
   double b = p.u.x() * p.p.x() + p.u.y() * p.p.y() - A_ * (p.p.z() - h_) * p.u.z();
   double c = p.p.sqLengthXY() - A_ * (p.p.z() - h_) * (p.p.z() - h_);
   double det = b * b - a * c; // Нет пересечения с конусом
   if (det < 0)
      return DBL_MAX;
   ...
   if (a == 0)
      ...
      if (d2 < 0)
         return DBL_MAX; // все пересечения сзади
...
void mcTransportCone::dumpVRML(ostream& os) const
{
   ...
   os << "Transform {" << endl;
   os << "  translation " << p.x() << ' ' << p.y() << ' ' << p.z() << endl; //os << "  rotation 1 0 0 1.5708" << endl;
