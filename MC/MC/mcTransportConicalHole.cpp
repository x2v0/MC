...
mcTransportConicalHole::mcTransportConicalHole(void) : mcTransport()
{
   setGeometry(0, 0, 0, 0);
}

mcTransportConicalHole::mcTransportConicalHole(const geomVector3D& orgn, const geomVector3D& z, const geomVector3D& x,
                                               double r0, double r1, double h, double f) : mcTransport(orgn, z, x)
{
...
double mcTransportConicalHole::getDistanceOutside(mcParticle& p) const
{
   ...
   double dd = rr <= r0_
                  ...
                  : c.lengthXY() >= r1_
                  ? mcGeometry::getDistanceToInfiniteCylinderOutside(p.p, p.u, r1_)
                  : DBL_MAX;
   if (dd == DBL_MAX)
      return DBL_MAX;
   cd += dd;
