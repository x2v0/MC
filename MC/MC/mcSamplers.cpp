...
double mcSamplers::SampleGauss(double rnd)
{
   ...
   double f2 = pa + 0.5 * f1; // Между распределением гаусса и функцией ошибок.
   // Гаусс ~exp(-x^2/2)
   // Erf ~exp(-x^2).
   // Мы работаем Гауссом.
...
double mcSamplers::SampleExponent2D(double C)
{
   ...
   if (C < DBL_EPSILON)
      return 1.0 / DBL_EPSILON; // На каждом шаге решаем задачу пересечени параболы с прямой линией
   // Стартовая точка
   double y0 = 1;
   double x0 = log(y0 / C);
   double xstep = 1;
