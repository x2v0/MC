...
public:
   mcTransportRectanglePolygonSideHole(const geomVector3D& orgn, const geomVector3D& vz, const geomVector3D& vx,
                                       double dx, double dy, const std::vector<double>& z, const std::vector<double>& x,
...
protected: // Толщины камней (расстояние от внутренней грани до внешней), коллимирующих поле.
   double dx_;
   double dy_;             // Координаты полигона грани шторок
   ...
   double fsx2_;
   double fsy1_;
   double fsy2_; // Служебные переменые
