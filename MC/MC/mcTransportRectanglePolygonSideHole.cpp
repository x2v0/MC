...
mcTransportRectanglePolygonSideHole::mcTransportRectanglePolygonSideHole(
   const geomVector3D& orgn, const geomVector3D& vz, const geomVector3D& vx, double dx, double dy,
   const std::vector<double>& z, const std::vector<double>& x, const std::vector<double>& y) :
...
double mcTransportRectanglePolygonSideHole::getDistanceInside(mcParticle& p) const
{
   ...
   if (vz < 0) {
      ...
      if (x >= (-dx_ + fsx1_) && x <= (dx_ + fsx2_) && y >= (-dy_ + fsy1_) && y <= (dy_ + fsy2_) && (
             x <= (-x_.front() + fsx1_) || x >= (x_.front() + fsx2_) || y <= (-y_.front() + fsy1_) || y >= (
                y_.front() + fsy2_)))
         return cd;
   } else if (vz > 0) {
...
double mcTransportRectanglePolygonSideHole::getDNearInside(const geomVector3D& p) const
{
   ...
   if (x <= (xmax_ + fsx2_) && x >= (-xmax_ + fsx1_) && y <= (ymax_ + fsy2_) && y >= (-ymax_ + fsy1_))
      return 0;
   double dd = MIN(MIN(ZORP(x + dx_ - fsx1_), ZORP(dx_ + fsx2_ - x)),
                   MIN(ZORP(y + dy_ - fsy1_), ZORP(dy_ + fsy2_ - y)));
   dd = MIN(dd, ZORP(z));
...
void mcTransportRectanglePolygonSideHole::dump(ostream& os) const
{
   ...
   os << "Cosines:" << endl;
   os << "SinX:";
   for (i = 0; i < sinx_.size(); i++)
      os << "\t" << sinx_[i];
   os << endl;
   os << "CosX:";
   for (i = 0; i < cosx_.size(); i++)
      os << "\t" << cosx_[i];
   os << endl;
   os << "SinY:";
   for (i = 0; i < siny_.size(); i++)
      os << "\t" << siny_[i];
   os << endl;
   os << "CosY:";
   for (i = 0; i < cosy_.size(); i++)
      os << "\t" << cosy_[i];
   os << endl;
}
