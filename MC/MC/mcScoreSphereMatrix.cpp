...
mcScoreSphereMatrix::mcScoreSphereMatrix(const char* module_name, int nThreads, int np, int nm, double rmin,
                                         double rmax) : mcScore(module_name, nThreads), np_(np), nm_(nm), rmin_(rmin),
                                                        rmax_(rmax)
{
...
void mcScoreSphereMatrix::ScorePoint(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
                                     const geomVector3D& p)
{
   double r2 = p.sqLength();
   if (r2 < r2min_ || r2 > r2max_)
      return;
   geomVector3D pp(p);
   pp.normalize();
