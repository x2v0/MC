...
{
   int i;
   for (i = 0; i < static_cast<int>(xes_.size()); i++)
      delete xes_[i];
   for (i = 0; i < static_cast<int>(physics_.size()); i++)
      delete physics_[i];
}
...
{
   for (short i = 0; i < static_cast<short>(mnames_.size()); i++)
      if (mnames_[i] == mname)
         return i;
...
{
   if (idx >= static_cast<short>(xes_.size()))
      throw std::exception("Medium index for photons an electrons is too big");
   return static_cast<mcMediumXE*>(xes_[idx]);
}
   ...
   while (!is.fail()) {
      if (line.find("MEDIUM=") != string::npos) {
         ...
         for (i = 0; i < static_cast<int>(mnames_.size()); i++)
            if (mnames_[i] == line)
               break;
         if (i < static_cast<int>(mnames_.size())) {
            xes_[i]->name_ = line;
            static_cast<mcMediumXE*>(xes_[i])->read(is);
         }
      }
      getline(is, line, '\n');
...
{
   if (ptype >= static_cast<int>(physics_.size()))
...
{
   if (idx >= static_cast<int>(mnames_.size()))
      throw std::exception("Unsupported particle type");
   return xes_[idx];
}
