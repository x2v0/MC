...
mcSourceLEBA::mcSourceLEBA(const char* name, int nThreads, spectrum_distr_t sptype, profile_distr_t prf_type,
                           double kemean, double kewidth, double z, double rsigma) : mcSource(name, nThreads),
                                                                                     sptype_(sptype),
                                                                                     prf_type_(prf_type), q_(-1),
                                                                                     kemean_(kemean), kewidth_(kewidth),
                                                                                     z_(z), rsigma_(rsigma) {}
...
void mcSourceLEBA::sample(mcParticle& p, mcThread* thread)
{
   ...
   } else if (sptype_ == SPECTRUM_PRISM) {
   ...
   if (rsigma_ == 0)
      ...
      double r = 0;
      if (prf_type_ == PROFILE_PRISM)
         r = mcSamplers::SamplePrism(rng.rnd());
      else if (prf_type_ == PROFILE_GAUSS)
         r = 2.0 * mcSamplers::SampleGauss(rng.rnd());
      else if (prf_type_ == PROFILE_EXPONENT)
