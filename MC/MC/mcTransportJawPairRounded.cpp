...
mcTransportJawPairRounded::mcTransportJawPairRounded(const geomVector3D& orgn, const geomVector3D& z,
                                                     const geomVector3D& x, double r, double h, double dx) :
   mcTransport(orgn, z, x), r_(r), r2_(r * r), h_(h), dx_(dx) {}
...
double mcTransportJawPairRounded::getDistanceInside(mcParticle& p) const
{
   ...
   double dist = 0; // Приводим расчет только к одной половинке
