...
double mcPhysicsElectron::TakeOneStep(mcParticle* p, const mcMedium& med, double& step) const
{
   ...
   const mcMediumXE& m = (const mcMediumXE&)med;
   double e_dep; // «десь нет проблемы пересечени¤ границы. ќна уже решена раньше.
   ...
   if (iLogKE < 0)
      iLogKE = 0; // вблизи энергии поглощени¤ возможна проблема индексов
   dedx = p->regDensityRatio * (m.dedx0_nega[iLogKE] + logKE * m.dedx1_nega[iLogKE]);
   e_dep = dedx * pathLength;
   e_dep = MIN(e_dep, p->ke);
   p->p += p->u * step; // Change direction in accordance with the Moliere theory
   MoliereScatter(p->thread_->rng(), m.scaledLittleB, m.chi_cc, p->u, p->ke + EMASS, betaSquared,
...
void mcPhysicsElectron::MollerScatter(mcParticle* p, const mcMediumXE* med)
{
   ...
   mcRng& rng = p->thread_->rng(); // Get the energies of the two electrons
...
void mcPhysicsElectron::DoBremsstrahlung(mcParticle* p, const mcMediumXE* med)
{
   ...
   double ke_elec = ke_in - e_phot; // Instantiate a new particle and assign the appropriate charges,
...
void mcPhysicsElectron::GetMollerEnergies(mcRng& rng, const mcMediumXE* med, double ke_in, double* ke_elec1,
                                          double* ke_elec2)
{
   ...
   double ratio;                     // Compute parameters that depend only on ke_in
   ...
   double gmax = (1.0 + 1.25 * g2); // Loop until a value of r is accepted
   do {
      r = med->eventCutoff_elec / (ke_in - eAboveThreshold * rng.rnd());
      ratio = r / (1.0 - r);
   } while (gmax * rng.rnd() > (1.0 + g2 * SQUARE(r) + ratio * (ratio - g3))); // Divide up the energy
   (*ke_elec2) = r * ke_in;
...
double mcPhysicsElectron::BremsPhotonEnergy(mcRng& rng, const mcMediumXE* pMedium, double ke_in)
{
   ...
   for (;;) {
      ...
      if (rng.rnd() * (n_brems * pMedium->aOrB[idist] + 0.5) > 0.5) {
         ...
         iscrn = ascrn; // Sample from the (1-r)/r part of the distribution,
         ...
         iscrn = bscrn; // Sample from the 2*r part of the distribution
      ...
      if (e_phot >= pMedium->eventCutoff_phot && e_phot <= ke_in) {
         ...
         if (del < pMedium->delLimit[idist]) {
            ...
            if (delta < 1.0) {
               ...
               screenLevel = pMedium->screen0a[iscrn] + pMedium->screen1a[iscrn] *
                             log(delta + pMedium->screen2a[iscrn]);
            }
            if (rng.rnd() <= screenLevel)
               break;
         }
      }
   }
...
double mcPhysicsElectron::BremsRejectionFunction(double x, double zFactor, double r, double val1, double val2,
                                                 double val3)
{
   double temp = x + 1.0;
   temp *= temp;
...
double mcPhysicsElectron::BremsPhotonAngle(mcRng& rng, const mcMediumXE* pMedium, double ke_in, double e_phot)
{
   ...
   if (pMedium->bremsAngleOption == KOCH_MOTZ) {
      ...
      double x_max = SQUARE(y_max); // Calculate some parameters for repeated use by the rejection function g(x)
      double r = 1.0 - e_phot / e_in;
      double val1 = 1.0 + SQUARE(r);
      double val2 = 3.0 * val1 - 2.0 * r;
      double val3 = (1.0 - r) / (2.0 * r * gamma);
      val3 *= val3; // Compute N_r (see page 5 of PIRS-0203)
      ...
      double N_r = 1.0 / MAX(g_xmin, MAX(g_xmid, g_xmax)); // Loop until a value of x is accepted
      double x;
      double g_x; // Value of rejection function g(x)
      do {
         // x equals the value under the radical in equation (5) of PIRS-0203
         double rnd0 = rng.rnd();
         x = rnd0 / (1.0 - rnd0 + 1.0 / x_max); // Compute g(x): */
         g_x = BremsRejectionFunction(x, zFactor, r, val1, val2, val3);
      } while (rng.rnd() > N_r * g_x); // Convert the accepted value of x into an angle in radians
      theta = sqrt(x) / gamma;
   } else {
...
void mcPhysicsElectron::SetUpBremsPhoton(mcParticle* pPhoton, const mcMediumXE* med, double ke_in, double e_phot)
{
   ...
   pPhoton->q = 0;
   pPhoton->ke = e_phot; // Establish the new direction
