...
mcScoreParticleContainer(const char* module_name, int nThreads) : mcScore(module_name, nThreads),
...
{
   if (etotal_other_ != nullptr)
      delete[] etotal_other_;
}
...
void mcScoreParticleContainer::ScoreFluence(const mcParticle& particle)
{
   ...
   pr.x = static_cast<float>(particle.p.x());
   pr.y = static_cast<float>(particle.p.y());
   pr.z = static_cast<float>(particle.p.z());
   pr.ux = static_cast<float>(particle.u.x());
   pr.uy = static_cast<float>(particle.u.y());
   pr.uz = static_cast<float>(particle.u.z());
   pr.e = static_cast<float>(particle.ke);
   pr.r = static_cast<float>(particle.p.length());
   double a = particle.u.z();
   a = a < 0 ? 0 : a > 1 ? 1 : a;
   pr.a = static_cast<float>(acos(a) * 180 / PI);
...
void mcScoreParticleContainer::dumpStatistic(ostream& os) const
{
   ...
   for (it = 0; it < nThreads_; it++) {
      ...
      for (i = 0; i < pl.size(); i++) {
         ...
         if (eidx < ameans.size()) {
            if (static_cast<int>(eidx) > eidxmax)
               eidxmax = static_cast<int>(eidx);
            ameans[eidx] += p.a;
            acounts[eidx]++;
         }
      }
   ...
   for (it = 0; it < nThreads_; it++) {
      ...
      for (i = 0; i < pl.size(); i++) {
         const PlaneParticleRecord& p = pl[i];
         os << p.x << "\t" << p.y << "\t" << p.z << "\t" << p.ux << "\t" << p.uy << "\t" << p.uz << "\t" << p.e << "\t"
            << p.r << "\t" << p.a << endl;
      }
   }
   os << endl;
