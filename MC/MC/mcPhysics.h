...
class mcParticle;
class mcMedium;
class geomVector3D; // Базовый класс моделирования физических процессов
class mcPhysics
{
public:
   mcPhysics(void);

   virtual ~mcPhysics(void); // Расчет средней длины пробега в среде.
   ...
   virtual bool Discarge(mcParticle* p, const mcMedium& med, double& edep) const = 0; // Physics utilities
