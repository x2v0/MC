...
public:
   ...
   double getDNearInside(const geomVector3D& p) const override; // Нужно для тестирования
   double A() const
   {
      return a_;
   }
   ...
   const std::vector<double>& Ds() const
   {
      return ds_;
   }

   const std::vector<double>& Hs() const
   {
      return hs_;
   }
...
protected:    // Объект описывается как двумерная сетка цилиндрических отверстий (5х5)
   double a_; // сторона квадратного фантома
   double z_; // толщина фантома
   // Диаметры отверстий
   std::vector<double> ds_; // Глубины отверстий
   std::vector<double> hs_; // Позиции узлов сетки отверстий
   std::vector<double> xs_;
   std::vector<double> ys_; // Служебные переменные, определяющие где могут находиться высверленные отверстия
