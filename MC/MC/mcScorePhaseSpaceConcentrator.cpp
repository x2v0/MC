...
mcScorePhaseSpaceConcentrator::mcScorePhaseSpaceConcentrator(const char* module_name, int nThreads,
                                                             enum mc_particle_t ptype, bool isXray, double focus,
                                                             int ne, int nr, int nthet, int naxial, double emax,
                                                             double rmax, double thetmax, const char* stat_file):
   mcScore(module_name, nThreads), ptype_(ptype), ne_(ne), nr_(nr), nthet_(nthet), naxial_(naxial), focus_(focus),
   emax_(emax), rmax_(rmax), model_file_(stat_file)
{
   isxray_ = isXray ? 1 : 0;
   de_ = emax_ / ne_;
...
void normalizeDistribution(std::vector<double>& d)
{
   double f = 0;
   int i;
   for (i = 0; i < static_cast<int>(d.size()); i++)
      f += d[i];
   f = 1e6 / f;
   for (i = 0; i < static_cast<int>(d.size()); i++)
      d[i] *= f;
