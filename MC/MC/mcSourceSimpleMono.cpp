...
mcSourceSimpleMono::mcSourceSimpleMono(const char* name, int nThreads, mc_particle_t type, double ke,
                                       const geomVector3D& p, const geomVector3D& v) : mcSource(name, nThreads)
{
   init(type, ke, p, v);
}
...
void mcSourceSimpleMono::init(mc_particle_t type, double ke, const geomVector3D& p, const geomVector3D& v)
{
   type_ = type;
   ke_ = ke;
   p_ = p;
