...
class mcRng;
class mcThread;
class mcScoreTrack; // Базовый класс описания источника частиц для 
...
public:
   ...
   bool IsStartInside() const
   {
      return isStartInside_;
   };

   void SetIsStartInside(bool b)
   {
      isStartInside_ = b;
   };
   ...
   {
      os << static_cast<const mcObj&>(s);
      os << "Etotal = " << s.etotal() << endl;
      return os;
   }

protected:
   char attached_module_[64]; // Указатель на объект, собирающий перемещения частиц между модулями
   mcScoreTrack* trackScore_;
   int nThreads_;
   double* etotal_; // Если это гамма источник, то частицы рождаются внутри объекта.
