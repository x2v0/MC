...
mcScoreMatrixCylinderAzimut::mcScoreMatrixCylinderAzimut(const char* module_name, int nThreads, int nr, int na,
                                                         double rmax, double zmin, double zmax) : mcScore(
   module_name, nThreads)
{
   m_nr = nr;
   m_na = na;
   m_rmax = rmax;
...
void mcScoreMatrixCylinderAzimut::ScorePoint(double edep, int iThread, const mcRegionReference& region,
                                             mc_particle_t pt, const geomVector3D& p)
{
                                            ...
                                            const geomVector3D& p0, const geomVector3D& p1)
{
   ...
   v.normalize(); // “рек за пределами торцевых плоскостей скоринга целиком
   if (vz2.z() <= m_zmin || vz1.z() >= m_zmax)
      return; // ќбрезаем трек плоскост€ми торцов цилиндра.
   if (vz1.z() < m_zmin)
      vz1 += v * ((m_zmin - vz1.z()) / v.z());
   if (vz2.z() > m_zmax)
      vz2 += v * ((m_zmax - vz2.z()) / v.z()); // — этого момента задача становитс€ плоской.
   ...
   while (true) {
      ...
      if (ir < 0 && !isMoveInside)
         break; // ≈сли снаружи, то сначала определ€ем точку входа в скоринг.
      if (ir < 0) {
      ...
      // отрицательное f означает вылетание из конуса без пересечени€ его границ
...
void mcScoreMatrixCylinderAzimut::getVoxelAtPoint(const geomVector3D& p, int& ir, int& ia)
{
   ...
   if (y < 0)
      angle = 2 * PI - angle;
   ia = static_cast<int>(angle / m_astep);
   if (ia >= m_na) {
      ia = -1;
      return;
   }
...
void mcScoreMatrixCylinderAzimut::dumpVRML(ostream& os) const
{
   ...
   for (int iz = 0; iz < 2; iz++) {
      double z = iz == 0 ? m_zmin : m_zmax; //  онцентрические круги
      for (ir = 1; ir <= m_nr; ir++) {
