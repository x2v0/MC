...
mcTransportPrism::mcTransportPrism() : mcTransport()
{
   setGeometry(0, 0, 0);
}

mcTransportPrism::mcTransportPrism(const geomVector3D& orgn, const geomVector3D& vz, const geomVector3D& vx, double ax,
                                   double ay, double az) : mcTransport(orgn, vz, vx)
{
...
{
   // HACK!!
   // По непонятным причинам использование DNEAR приводит к проблемам вблизи поверхностей.
   // Это серьезный предмет для разбирательства, так как код ниже, похоже работает правильно
   // и причина где-то глубоко внутри движка транспорта.
   // Последнее чревато фатальными проблемами.
   return 0;
