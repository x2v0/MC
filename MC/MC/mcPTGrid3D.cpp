...
mcPTGrid3D::mcPTGrid3D(void) : mcTransport()
{
   setGeometry(0, 0, 0, 0);
}

mcPTGrid3D::mcPTGrid3D(const geomVector3D& orgn, const geomVector3D& z, const geomVector3D& x, int nc, double dr,
                       double ds, double h) : mcTransport(orgn, z, x)
{
...
void mcPTGrid3D::setGeometry(int nc, double dr, double ds, double h)
{
   nc_ = nc;
   dr_ = dr;
   ds_ = ds;
...
double mcPTGrid3D::getDistanceInside(mcParticle& p) const
{
   // Цилиндр
...
double mcPTGrid3D::getDistanceOutside(mcParticle& p) const
{
...
double mcPTGrid3D::getDNearInside(const geomVector3D& p) const
{
   double z = p.z();
   double r = p.lengthXY();
   int ir = int(r / ds_ + MC_EPSILON);
...
void mcPTGrid3D::dumpVRML(ostream& os) const
{
