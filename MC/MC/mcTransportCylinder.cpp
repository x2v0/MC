...
mcTransportCylinder::mcTransportCylinder(const geomVector3D& orgn, const geomVector3D& z, const geomVector3D& x,
                                         double r, double h) : mcTransport(orgn, z, x)
{
   setGeometry(r, h);
}
...
{
   dumpVRMLCylinder(os, r_, 0, h_); //geomVector3D p = geomVector3D(0, 0, h_*0.5) * mttow_;
