...
void mcTransport::beginTransport(mcParticle& p)
{
   ...
   p.transport_ = this; // Объекты транспорта могут ставить печать в момент											
   ...
   *particle = p;
   ...
   if (score_)
      score_->ScoreFluence(*particle); // Транспорт в локальной системе координат
   simulate(p.thread_);
...
void mcTransport::endTransport(mcParticle* particle)
{
   ...
   pp.u = pp.u.transformDirection(mttow_); // Если пересекаем внешнюю поверхность и нет охватывающего объекта, 
...
mc_move_result_t mcTransport::moveParticle(mcParticle* particle, double& step, double& edep)
{
   ...
   if (particle->region.idx_ == 0) {
      ...
      if (step == DBL_MAX) {
         // промазали, летим в следующий слой
   ...
   if (isMultiRegions_) {
      ...
      // Новый трик с тем, чтобы частица чуть-чуть заступала за границу;
   } // HACK!!
                               ...
                               double y0) const
{
   ...
   double f2 = normPositive ? r2 : r1; // Преобразование из системы объекта в мировую систему 
                                        ...
                                        double h) const
{
   ...
   os << "                1, 9, 10, 2, -1," << endl;
   os << "                2, 10, 11, 3, -1," << endl;
   os << "                3, 11, 8, 0, -1," << endl; // Внутренняя боковая поверхность
   os << "                4, 5, 13, 12, -1," << endl;
   os << "                5, 6, 14, 13, -1," << endl;
   os << "                6, 7, 15, 14, -1," << endl;
   os << "                7, 4, 12, 15, -1," << endl; // Нижний торец
   os << "                0, 1, 5, 4, -1," << endl;
   os << "                1, 2, 6, 5, -1," << endl;
   os << "                2, 3, 7, 6, -1," << endl;
   os << "                3, 0, 4, 7, -1," << endl; // Верхний торец
   os << "                8, 12, 13, 9, -1," << endl;
