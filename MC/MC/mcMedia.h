...
public:
   ...
   ~mcMedia(void); // Добавление сред по названиям.
   ...
   void addName(const char* mname); // Поиск среды по имени, полезный для инициализации транспортов
   ...
   vector<mcMedium*> Media()
   {
      return xes_;
   }

protected:                 // Список имен сред
   vector<string> mnames_; // Параметры сред транспорта фотонов и электронов
