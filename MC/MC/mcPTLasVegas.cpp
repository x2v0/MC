...
mcPTLasVegas::mcPTLasVegas(const geomVector3D& orgn, const geomVector3D& z, const geomVector3D& x) :
   mcTransport(orgn, z, x), a_(14.0), z_(2.175), ds_(6), hs_(5), xs_(6), ys_(5), x1_(0), x2_(0), y1_(0), y2_(0), z2_(0)
{
   ds_[0] = 0.1;
   ds_[1] = 0.2;
   ds_[2] = 0.4;
   ...
   ax_ = 2.0 * MAX(-x1_, x2_);
   ay_ = 2.0 * MAX(-y1_, y2_); //// Тест со значительно большими толщинами
...
double mcPTLasVegas::getDistanceInside(mcParticle& p) const
{
   ...
   if ((fabs(x) <= ax_ / 2 && fabs(y) <= ay_ / 2 && fabs(z) <= z2_) || mcGeometry::getDistanceToPrismOutside(
          p.p, p.u, ax_, ay_, z2_) != DBL_MAX) {
      geomVector3D pp = p.p; // Проверяем пересечение с цилиндрами
      double dcyl = dprism;
      unsigned i, j;
      for (j = 0; j < ys_.size(); j++) {
         pp(1) = p.p.y() - ys_[j];
...
double mcPTLasVegas::getDistanceOutside(mcParticle& p) const
{
   ...
   if (ispinside) {
      unsigned i, j; // Ищем цилиндр
      for (i = 0; i < xs_.size(); i++)
         if (fabs(x - xs_[i]) <= ds_[i] / 2)
            break;
      if (i >= xs_.size())
         throw std::exception(
            "mcPTLasVegas::getDistanceOutside: Particle expected to be inside cylinder, but cylinder not found");
      for (j = 0; j < ys_.size(); j++)
         if (((x - xs_[i]) * (x - xs_[i]) + (y - ys_[j]) * (y - ys_[j]) - FLT_EPSILON) * 4 <= ds_[i] * ds_[i])
            break;
      if (j >= ys_.size())
         throw std::exception(
            "mcPTLasVegas::getDistanceOutside: Particle expected to be inside cylinder, but cylinder not found");
   ...
   if (zz > z_ / 2 || xx < x1_ || xx > x2_ || yy < y1_ || yy > y2_)
      return dprism; // Перебираем круги на предмет нахождения в каком либо.
   ...
   if (i > 0 && j > 0) {
      ...
      if (fx * fx + fy * fy <= ds_[i - 1] * ds_[i - 1] / 4) {
         ii = i - 1;
         jj = j - 1;
      }
   }
...
double mcPTLasVegas::getDNearInside(const geomVector3D& p) const
{
   ...
   return 0; //// Сначала определяем минимальное расстояние до параллелепипеда.
	//// Затем перебираем 4 соседних цилиндра.
	//// В идоге выбираем меньшее. //double x = p.x(), y = p.y(), z = p.z();
