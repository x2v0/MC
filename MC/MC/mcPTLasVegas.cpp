   ...
   mcTransport(orgn, z, x), a_(14.0), z_(2.175), ds_(6), hs_(5), xs_(6), ys_(5), x1_(0), x2_(0), y1_(0), y2_(0), z2_(0)
{
   ...
   ax_ = 2.0 * MAX(-x1_, x2_);
   ay_ = 2.0 * MAX(-y1_, y2_); //// “ест со значительно большими толщинами
...
double mcPTLasVegas::getDistanceInside(mcParticle& p) const
{
   ...
   if ((fabs(x) <= ax_ / 2 && fabs(y) <= ay_ / 2 && fabs(z) <= z2_) || mcGeometry::getDistanceToPrismOutside(
      ...
      geomVector3D pp = p.p; // ѕровер€ем пересечение с цилиндрами
      double dcyl = dprism;
      unsigned i, j;
      for (j = 0; j < ys_.size(); j++) {
         pp(1) = p.p.y() - ys_[j];
...
double mcPTLasVegas::getDistanceOutside(mcParticle& p) const
{
   ...
   if (zz > z_ / 2 || xx < x1_ || xx > x2_ || yy < y1_ || yy > y2_)
      return dprism; // ѕеребираем круги на предмет нахождени€ в каком либо.
...
double mcPTLasVegas::getDNearInside(const geomVector3D& p) const
{
   ...
   return 0; //// —начала определ€ем минимальное рассто€ние до параллелепипеда.
	//// «атем перебираем 4 соседних цилиндра.
	//// ¬ идоге выбираем меньшее. //double x = p.x(), y = p.y(), z = p.z();
