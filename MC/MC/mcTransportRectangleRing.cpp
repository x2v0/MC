...
mcTransportRectangleRing::mcTransportRectangleRing(const geomVector3D& orgn, const geomVector3D& vz,
                                                   const geomVector3D& vx, double x1, double x2, double y1, double y2,
                                                   double d, double h) : mcTransport(orgn, vz, vx), d_(d), h_(h),
...
double mcTransportRectangleRing::getDistanceOutside(mcParticle& p) const
{
   ...
   double dd = isXYInsideField(c.x(), c.y())
                  ...
                  ? mcGeometry::getDistanceToRectanglePipeOutside(p.p, p.u, x1_ - d_, x2_ + d_, y1_ - d_, y2_ + d_)
                  : DBL_MAX;
   if (dd == DBL_MAX)
      return DBL_MAX;
   cd += dd;
...
bool mcTransportRectangleRing::isXYInside(double x, double y) const
{
   ...
   return x > X1 && x < X2 && y > Y1 && y < Y2 && !(x >= x1_ && x <= x2_ && y >= y1_ && y <= y2_);
}
...
double mcTransportRectangleRing::distanceToRectangleInside(double x, double y, double X1, double X2, double Y1,
                                                           double Y2) const
{
   return ZORP(MIN(MIN(x - X1, X2 - x), MIN(y - Y1, Y2 - y)));
}

double mcTransportRectangleRing::distanceToRectangleOutside(double x, double y, double X1, double X2, double Y1,
                                                            double Y2) const
{
   if (x <= X1) {
      if (y < Y1)
