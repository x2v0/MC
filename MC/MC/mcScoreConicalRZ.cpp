...
mcScoreConicalRZ::mcScoreConicalRZ(const char* module_name, int nThreads, int nr, int nz, double rmax, double zmin,
                                   double zmax, double ziso, double sad) : mcScore(module_name, nThreads)
{
   m_nr = nr;
   m_nz = nz;
   m_rmax = rmax;
   ...
   for (int i = 1; i < nThreads_; i++)
      m_M[i] = m_M[i - 1] + m_nr * m_nz; //Sigma = new double[nThreads];
}
...
void mcScoreConicalRZ::ScorePoint(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
...
void mcScoreConicalRZ::ScoreLine(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
                                 const geomVector3D& p0, const geomVector3D& p1)
{
   ...
   v.normalize(); // Обрезаем трек плоскостями торцов цилиндра.
   ...
   if (rr >= m_rmax) {
      ...
      double d = mcGeometry::getDistanceToConeOutside(geomVector3D(vz1.x(), vz1.y(), m_iso - vz1.z()),
                                                      geomVector3D(v.x(), v.y(), -v.z()), m_rmax, m_sad);
      if (d == DBL_MAX)
   ...
   if (ir >= m_nr)
      ir = m_nr - 1; // Плотность потерь энергии
   double eddens = edep / (p1 - p0).length();
   int irnext = ir;
   while ((vz2 - vz1) * v > DBL_EPSILON) {
      ir = irnext;
      double r = ir * m_rstep;
      double d = DBL_MAX; // Если есть пересечение с внутренним цилиндром, то мы движемся на него а не от него
      // и ближайшее пересечение с внутренним а не внешним.
      if (ir > 0)
         d = mcGeometry::getDistanceToConeOutside(geomVector3D(vz1.x(), vz1.y(), m_iso - vz1.z()),
                                                  geomVector3D(v.x(), v.y(), -v.z()), r, m_sad);
      // Если пересекаем внутренний конус, то заведомо остаемся внутри внешнего.
      if (d == DBL_MAX) {
         // Прорабатываем движение наружу
         d = mcGeometry::getDistanceToConeInside(geomVector3D(vz1.x(), vz1.y(), m_iso - vz1.z()),
                                                 geomVector3D(v.x(), v.y(), -v.z()), r + m_rstep, m_sad);
         irnext++;
      } else
         irnext--;
      if (d < 0)
      ...
      if ((vz12 - vz2) * v > 0)
         vz12 = vz2; // Текущий слой и граница следующего
      int iz = int((vz1.z() - m_zmin) / m_zstep);
      double z2 = m_zmin + m_zstep * (iz + 1); // Если конец отрезка в том же слое, то и не надо перебирать
      if (vz12.z() < z2)
...
void mcScoreConicalRZ::scoreEnergyInVoxel(int iThread, int ir, int iz, double edep)
{
   ...
   if (ir < 0) {
      ...
      s << "ir = \t" << ir << std::endl;
      s << "iz = \t" << iz << std::endl;
      s << "edep = \t" << edep << std::endl; //throw std::exception(s.str());
      cout << s.str();
      return;
   }
...
//суммарная доза по батчам (средняя доза умноженная на кол-во потоков)
{
   double f = 0;
   if (iz >= 0 && iz < m_nz && ir >= 0 && ir < m_nr) {
      for (int i = 0; i < nThreads_; i++)
...
void mcScoreConicalRZ::dumpStatistic(ostream& os) const
{
   ...
   os << nr << '\t' << nz << '\t' << rmax << '\t' << zmin << '\t' << zmax << '\t' << m_iso << '\t' << m_sad << endl <<
      endl; // this code - for save transpose matrix
   for (ir = 0; ir < nr; ir++)
   ...
   os << endl;
   for (iz = 0; iz < nz; iz++) {
      os << zmin + (double(iz) + 0.5) * zstep; //"================Sigma==================";
      for (ir = 0; ir < nr; ir++)
         os << '\t' << Sigma(ir, iz);
      os << endl;
   }
   ...
   os << endl; // Версия с подавлением шумов.
