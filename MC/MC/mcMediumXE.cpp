...
mcMediumXE::mcMediumXE(void) : iLogKE0_phot(0), iLogKE1_phot(0), eventCutoff_phot(0), rayleigh(0), e_KEdge(0),
                               iLogKE0_rayl(0), iLogKE1_rayl(0), iLogKE0_elec(0), iLogKE1_elec(0), eventCutoff_elec(0),
                               radLength(0), teff0(0), eStep(0.2), scaledLittleB(0), chi_cc(0), del_C(0),
                               zFactor_angDist(0), mollerThreshold(0), transCutoff_phot(0.05), transCutoff_elec(0.2),
                               ke_noRangeDisc(0), bremsAngleOption(KOCH_MOTZ), pairAngleOption(MOTZ_OLSEN_KOCH),
...
void mcMediumXE::read(istream& is)
{
   ...
   this->mollerThreshold = 2.0 * this->eventCutoff_elec; // Line
   getline(is, line, '\n');
   if (is.fail())
      return;
   ...
   if (GetFloatArray(line, a) != 36)
      throw std::exception("36 parameters expected");
   j = 0;
   for (i = 0; i < 6; i++) {
      this->screen0[i] = a[j++];
      this->screen1[i] = a[j++];
   ...
   if (is.fail())
      return;
   line += s1;
   if (GetFloatArray(line, a) != 7)
      throw std::exception("7 parameters expected");
   this->del_C = a[0];
   this->aOrB[0] = a[1];
   this->aOrC[0] = a[2];
   this->delLimit[0] = a[3];
   this->aOrB[1] = a[4];
   this->aOrC[1] = a[5];
   this->delLimit[1] = a[6]; // Read electron data:
   // Line
   getline(is, line, '\n');
   if (is.fail())
      return;
   ...
   if (is.fail())
      return;
   line += s1;
   if (GetFloatArray(line, a) != 6)
      throw std::exception("6 parameters expected");
   this->teff0 = a[1];
   ...
   if (is.fail())
      return;
   if (GetFloatArray(line, a) != 3)
      throw std::exception("3 parameters expected");
   this->e_KEdge = a[0];
   ...
   if (this->rayleigh > 0) {
      ...
      this->raylFactor1.resize(nBins_phot); // Line
      getline(is, line, '\n');
      if (is.fail())
         return;
      int nBins_rayl = atoi(line.c_str()); // Line
      getline(is, line, '\n');
      if (is.fail())
         return;
      if (GetFloatArray(line, a) != 2)
         throw std::exception("2 parameters expected");
      this->iLogKE0_rayl = a[0];
      this->iLogKE1_rayl = a[1];
      this->raylQFactor0.resize(nBins_rayl);
      this->raylQFactor1.resize(nBins_rayl); // QFactors
      n = (nBins_rayl * 2 + 4) / 5;
      line.clear();
      for (i = 0; i < n; i++) {
         // Line
   ...
   this->chi_cc *= sqrt(inverseDistanceFactor); // Correct for the fact that PEGS assumes matrix indices
   // begin with one, whereas they begin with zero in C:
   this->iLogKE0_elec -= 1.0;
   this->iLogKE0_phot -= 1.0;
   if (this->rayleigh > 0)
      this->iLogKE0_rayl -= 1.0; // Fix step sizes (note that the differing dedx arrays for negatrons
   // and positrons here give rise to differing stepSize arrays):
   FixStepSize(nBins_elec, this->eStep, this->iLogKE0_elec, this->iLogKE1_elec, this->stepSize0_nega,
               this->stepSize1_nega, this->dedx0_nega, this->dedx1_nega);
   FixStepSize(nBins_elec, this->eStep, this->iLogKE0_elec, this->iLogKE1_elec, this->stepSize0_posi,
               this->stepSize1_posi, this->dedx0_posi, this->dedx1_posi);
...
void mcMediumXE::FixStepSize(int nBins_elec, double eStep, double iLogKE0, double iLogKE1, vector<double>& stepSize0,
                             vector<double>& stepSize1, vector<double>& dedx0, vector<double>& dedx1)
{
   ...
   for (int i = 0; i < nBins_elec - 1; i++) {
      ...
      double step = eStep * exp(logKE) / dedx; // Solve the equations:
...
void mcMediumXE::InitializeAngularDistribution()
{
   ...
   for (int i = 0; i < static_cast<int>(this->elements_.size()); i++) {
      this->zFactor_angDist += this->elements_[i].partsByNumber * this->elements_[i].atomicNumber * (
