...
mcSourceDistributed::mcSourceDistributed(void) : mcSource(), distr_(), Emitx_(0), rbeamx_(0), rbetax_(0),
                                                 beamAnglex_(0), Emity_(0), rbeamy_(0), rbetay_(0), beamAngley_(0),
                                                 beta0_(0), type_(MCP_PHOTON), ke_(0), q_(0) {}

mcSourceDistributed::mcSourceDistributed(const char* name, int nThreads, mc_particle_t type, double ke,
                                         const geomVector3D& p, const geomVector3D& v, mc_distr_t distr, double Emitx,
                                         double rbeamx, double beamAnglex, double Emity, double rbeamy,
                                         double beamAngley) : mcSource(name, nThreads)
{
...
)
{
   ...
   // если источник не протонный - исправить массу!!!!! Предполагается, что ke_ задано в МэВ-ах
...
void mcSourceDistributed::sample(mcParticle& p, mcThread* thread)
{
   ...
   mcRng& rng = thread->rng();          // если однородный по плотности или гауссов - готовим однородный круг в x,y
   if (distr_ == MCP_CONST_DENS || distr_ == MCP_GAUSSIAN) {
      double r2, rbeta2, spread;
      do {
         x = (rng.rnd() - 0.5) * rbeamx_ * 2;
         y = (rng.rnd() - 0.5) * rbeamy_ * 2;
      } while (x * x / rbeamx_ / rbeamx_ + y * y / rbeamy_ / rbeamy_ > 1); // если гауссов  - растягиваем созданный круг
      ...
      do {
         ...
         py = (rng.rnd() - 0.5) * 2 * rbetay_;
                  ...
                  1 - x * x / rbeamx_ / rbeamx_ - y * y / rbeamy_ / rbeamy_)); //для гауссова пучка
      if (distr_ == MCP_GAUSSIAN) {
         do {
            px = (rng.rnd() - 0.5) * 2 * rbetax_;
            py = (rng.rnd() - 0.5) * 2 * rbetay_;
   ...
   if (distr_ == MCP_WATERBAG || distr_ == MCP_K_V) {
      ...
      } while ((Rx2 + Ry2) > 1); // waterbag легко преобразуется к распределению Капчинского-Владимирского
   ...
   if (distr_ == MCP_ARBITRARY) {
      //  произвольное распределение, пока только демонстрация алшоритма
      double Rx2, Ry2;
      do {
         do {
            x = (rng.rnd() - 0.5) * rbeamx_ * 2;
   ...
   py += beta0_ * beamAngley_ / rbeamy_ * y; // готовим частицу
   p.t = type_;
   p.q = q_;
   p.ke = ke_;
   p.p = p_ + geomVector3D(x, y, 0); // углы считаем малыми
