...
mcSourceModelRadialPhsp::mcSourceModelRadialPhsp(const char* name, int nThreads, double z0): mcSource(name, nThreads),
                                                                                             isxray_(0), z0_(z0),
                                                                                             ne_(0), nr_(0), nthet_(0),
                                                                                             naxial_(0), focus_(0),
                                                                                             emax_(0), rmax_(0),
                                                                                             thetmax_abs_(0),
                                                                                             thetmax_(0), de_(0),
...
void mcSourceModelRadialPhsp::sample(mcParticle& p, mcRng& rng) const
{
   ...
   if (rnd < we117_ && isxray_ == 0) {
      ...
      if (eidx >= ne_ - 1)
         eidx = ne_ - 2;
      int eidx2 = eidx + 1;
      f = p.ke / de_ - double(eidx); // GG 20160212 После предположения о том, что проблемы могут создаваться
...
void mcSourceModelRadialPhsp::sampleForEnergy(int eidx, double& r, double& t, double& a, double rnd0, double rnd1,
                                              double rnd2) const
{
   double f = rhistograms_[eidx].sample(rnd0);
   int ridx = int(f / dr_), ridx2 = ridx + 1; // Экстраполяция может приводить к фатальным проблеммам
   if (ridx2 >= nr_)
      ridx2 = nr_ - 1;
   double fr = f / dr_ - double(ridx);
   r = rmax_ * acos(1 - f); // GG 20160212 Отказ от интерполяции.
...
void* mcSourceModelRadialPhsp::saveToMemory(int& size)
{
   ...
   *(int*)pbuffer = naxial_;
   pbuffer += sizeof(int);
   *(double*)pbuffer = focus_;
   pbuffer += sizeof(double);
   *(double*)pbuffer = emax_;
   pbuffer += sizeof(double);
   *(double*)pbuffer = rmax_;
   pbuffer += sizeof(double);
   *(double*)pbuffer = thetmax_abs_;
   pbuffer += sizeof(double);
   *(double*)pbuffer = thetmax_;
   pbuffer += sizeof(double);
   *(double*)pbuffer = we117_;
   pbuffer += sizeof(double);
   *(double*)pbuffer = we133_;
   pbuffer += sizeof(double);
   *(double*)pbuffer = wexscale_;
   pbuffer += sizeof(double);
...
void mcSourceModelRadialPhsp::readFromMemory(void* buffer)
{
   ...
   xehistogram_.restore(ne_ + 1, (double*)(p));
   p += (ne_ + 1) * sizeof(double);
   int ie, ne = isxray_ == 0 ? ne_ + 2 : ne_;
   for (ie = 0; ie < ne; ie++) {
      rhistograms_[ie].restore(nr_ + 1, (double*)(p));
      p += (nr_ + 1) * sizeof(double);
   }
   for (ie = 0; ie < ne; ie++) {
      for (int ir = 0; ir < nr_; ir++) {
         thistograms_[ie][ir].restore(nthet_ + 1, (double*)(p));
         p += (nthet_ + 1) * sizeof(double);
      }
   }
   for (ie = 0; ie < ne; ie++) {
      for (int ir = 0; ir < nr_; ir++) {
...
#else
            ahistograms_[ie][ir][it].restore(naxial_ + 1, (double*)(p));
            p += (naxial_ + 1) * sizeof(double);
#endif
         }
      }
   }
...
void mcSourceModelRadialPhsp::createFromDistribution(int isxray, int ne, int nr, int nt, int na, double emax,
                                                     double rmax, double tmax, double focus, const double* data)
{
   isxray_ = isxray;
...
void mcSourceModelRadialPhsp::fitAsimutDistribution(int np, const double* data, double& a, double& b)
{
   ...
   if (c[np - 1] > 0) {
      double f = 1.0 / c[np - 1];
      for (i = 0; i < np; i++)
         c[i] *= f; // Подгоняем методом наименьших квадратов функцию
