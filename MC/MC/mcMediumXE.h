...
enum { BREMS_ANGLE_DEFAULT, KOCH_MOTZ };                    // Pair production angular distribution options:
enum { PAIR_ANGLE_DEFAULT, LOWEST_ORDER, MOTZ_OLSEN_KOCH }; // Photoelectron angular distribution options:
...
protected:
   static void FixStepSize(int nBins_elec, double eStep, double iLogKE0, double iLogKE1, vector<double>& stepSize0,
...
public: // Photons:
   // Log energy index:
   double iLogKE0_phot;
   double iLogKE1_phot; // Interactions:
   ...
   vector<double> raylFactor1; // Pair production:
   // The following data are unique to pair production:
   double aOrC[2]; // Compton scattering:
   // No medium data needed
   // Photoelectric effect:
   double e_KEdge; // Rayleigh scattering:
   double iLogKE0_rayl;
   double iLogKE1_rayl;
   vector<double> raylQFactor0;
   vector<double> raylQFactor1; // Electrons (negatrons or positrons):
   // Log energy index:
   double iLogKE0_elec;
   double iLogKE1_elec;     // Interactions:
   double eventCutoff_elec; // Transport:
   double radLength;        // Radiation length
   double teff0;
   double eStep; // Moliere multiple scattering:
   double scaledLittleB;
   double chi_cc; // Bremsstrahlung:
   // The following data are unique to bremsstrahlung:
   double aOrB[2]; // The following data are shared with the pair production routines:
   double screen0[6];
   double screen1[6];
   double screen2[6];
   double screen0a[6];
   ...
   vector<double> br11_nega;   // Transport:
   ...
   vector<double> stepSize1_nega; // Moller scattering:
   double mollerThreshold;        // Positrons:
   // Interactions:
   vector<double> br10_posi; // Branching ratio 1
   vector<double> br11_posi;
   vector<double> br20_posi;   // Branching ratio 2
   vector<double> br21_posi;   // Transport:
   ...
   vector<double> stepSize1_posi; // Bhabha scattering:
   // No medium data needed 
