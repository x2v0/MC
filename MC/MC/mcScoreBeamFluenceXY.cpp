...
mcScoreBeamFluenceXY::mcScoreBeamFluenceXY(const char* module_name, int nThreads, int nx, int ny, double psx,
                                           double psy, int nebins, double emax) : mcScore(module_name, nThreads),
                                                                                  nx_(nx), ny_(ny), psx_(psx),
                                                                                  psy_(psy), nebins_(nebins),
                                                                                  emax_(emax)
{
   minx_ = -0.5 * nx_ * psx_;
   miny_ = -0.5 * ny_ * psy_;
...
{
   if (intencity_all_)
      delete[] intencity_all_;
   if (intencity_)
      delete[] intencity_;
   if (spectrum_all_)
      delete[] spectrum_all_;
   if (spectrum_)
      delete[] spectrum_;
}
