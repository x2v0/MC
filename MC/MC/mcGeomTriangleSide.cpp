...
mcGeomTriangleSide::mcGeomTriangleSide(const geomVector3D& p, const geomVector3D& Vx, const geomVector3D& Vy, double ax,
                                       double ay) : P_(p), Vx_(Vx), Vy_(Vy), ax_(ax), ay_(ay)
{
   ...
   N_ = Vx_ ^ Vy_;
   N_.normalize();
   VRy_ = N_ ^ Vx_; // вершин треугольника
   p_[0].set(0, 0);
   p_[1].set(ax_, 0);
   p_[2].set(ay * (Vx_ * Vy_), ay * (VRy_ * Vy_)); // векторы и нормали ребер
   for (int i = 0; i < 3; i++) {
      v_[i] = p_[(i + 1) % 3] - p_[i];
      v_[i].normalize();
      n_[i] = v_[i];
...
double mcGeomTriangleSide::getDistance(const geomVector3D& p, const geomVector3D& v, bool inside) const
{
   ...
   geomVector3D pc = p + (v * distance); // Координаты точки проекции в системе треугольника
   geomVector3D pr = pc - P_;
   geomVector2D R(pr * Vx_, pr * VRy_); // Высоты к ребрам
   int i;
   double hr[3];
   bool isInside = true;
   for (i = 0; i < 3; i++) {
   ...
   if (isInside)
      return distance; // в нужном направлении но мимо треугольника
   return DBL_MAX;
}
...
double mcGeomTriangleSide::getDNear(const geomVector3D& p) const
{
   ...
   geomVector3D pp = p + (N_ * h); // Координаты точки проекции в системе треугольника
   ...
   if (isInside)
      return fabs(h); // Минимальным будет минимальное расстояние до одного из ребер.
