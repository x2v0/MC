...
mcHistogram::mcHistogram() : comulative_(false), np_(0), nmax_(0), min_(0), max_(0), step_(0), total_(0), sum_(0),
                             sum2_(0) {}

mcHistogram::mcHistogram(double min, double step, int np) : comulative_(false), nmax_(0), max_(0), total_(0), sum_(0),
                                                            sum2_(0), d_(np, 0)
{
   init(min, step, np);
}

mcHistogram::mcHistogram(const mcHistogram& h) : comulative_(h.comulative_), np_(h.npnt()), nmax_(h.nmax()),
                                                 min_(h.argmin()), max_(h.argmax()), step_(h.step()), total_(h.total()),
                                                 sum_(h.sum()), sum2_(h.sum2()), d_(h.d()) {}
...
{
   unsigned int i, j, nh = static_cast<int>(ha.size());
   if (!nh)
