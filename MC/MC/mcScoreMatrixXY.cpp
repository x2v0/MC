...
mcScoreMatrixXY::mcScoreMatrixXY(const char* module_name, int nThreads, int nx, int ny, double psx, double psy,
                                 double psz, double z0) : mcScore(module_name, nThreads), nx_(nx), ny_(ny), z0_(z0),
...
void mcScoreMatrixXY::ScorePoint(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
                                 const geomVector3D& p)
{
   ...
   if (ix >= nx_ || iy >= ny_)
      return;
   if (edep > 20.0) {
      cout << "Unexpected energy deposition at point x = " << p.x() << ",  y = " << p.y() << ",  z = " << p.z() <<
         ",  edep = " << edep << endl;
      return;
   }
...
void mcScoreMatrixXY::ScoreLine(double edep, int iThread, const mcRegionReference& region, mc_particle_t pt,
                                const geomVector3D& p0, const geomVector3D& p1)
{
   double x1 = p0.x(), y1 = p0.y(), z1 = p0.z();
   double x2 = p1.x(), y2 = p1.y(), z2 = p1.z(); // Make program faster
   ...
   if ((ix1 >= nx_ && ix2 >= nx_) || (iy1 >= ny_ && iy2 >= ny_))
      return; // When path is completely within one voxel
   if (ix1 == ix2 && iy1 == iy2 && z1 >= z0_ && z2 >= z0_ && z1 <= z1_ && z2 <= z1_) {
   ...
   if (edep > 20.0) {
      cout << "Unexpected energy deposition at line begin x = " << x1 << ",  y = " << y1 << ",  z = " << z1 <<
         ",  edep = " << edep << endl;
      return;
   }                                     // Energy lose per unit path.
   double f = edep / (p1 - p0).length(); // Go trough voxels
...
void mcScoreMatrixXY::SetImageFile(const char* fname)
{
   image_file_ = fname;
}

void mcScoreMatrixXY::SetCalibrationFile(const char* fname)
{
   calibration_file_ = fname;
}
      ...
      for (j = 0; j < ny_; j++) {
         for (i = 0; i < nx_; i++) {
            if (useCalibration && cmax > 0) {
               unsigned short c = calibr[j * nx_ + i];
               if (c <= 0)
                  c = static_cast<unsigned short>(cmax);
               data[j * nx_ + i] = static_cast<unsigned short>((Dose(i, j) * scale * cmax) / c);
            } else
               data[j * nx_ + i] = static_cast<unsigned short>(Dose(i, j) * scale);
         }
      }
...
void mcScoreMatrixXY::dumpVRML(ostream& os) const
{
   ...
   os << "      point [" << endl; // Грани XY (четные точки на Zmin, нечетные - Zmax)
   unsigned ncount = 0;
   for (j = 0; j < nj; j++) {
      if (j != 0 && j != nj - 1)
         continue; // Пропускаем внутренние линии. Рисуем только периметр
