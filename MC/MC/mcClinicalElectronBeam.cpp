...
void mcClinicalElectronBeam::sample(mcParticle& p, mcThread* thread)
{
   ...
   if (fabs(f) > 2)
      f = 0; // Таких частиц мало. Просто подменяем их монохроматичными, чтобы не усложнять код
...
void mcClinicalElectronBeam::dumpVRML(ostream& os) const
{
   ...
   os << "# Source: " << name_ << endl; // Шарик с упирающейся стрелкой
   os << "Transform {" << endl;
   ...
   os << "    }" << endl;
   os << "  ]" << endl;
   os << "}" << endl; // Рамка электронного аппликатора
