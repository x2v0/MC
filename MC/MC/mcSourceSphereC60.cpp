...
mcSourceSphereC60::mcSourceSphereC60(const char* name, int nThreads, const geomVector3D& p, const geomVector3D& v,
                                     double r) : mcSource(name, nThreads), p_(p), v_(v), r_(r)
{
   isGamma_ = true;
}
...
void mcSourceSphereC60::sample(mcParticle& p, mcThread* thread)
{
   ...
   p.t = MCP_PHOTON;
   p.q = 0;
   p.ke = rng.rnd() < 0.5 ? 1.33 : 1.17; // По радиусу распределение пропорционально кубическому корню.
