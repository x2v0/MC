...
mcScoreSpectraFluence::mcScoreSpectraFluence(const char* module_name, int nThreads, mc_particle_t pt, double ecut,
                                             int nr, int ne, double rmax, double emax) : mcScore(module_name, nThreads),
                                                                                         pt_(pt), nr_(nr),
                                                                                         rstep_(rmax / nr), rmax_(rmax),
                                                                                         ne_(ne), estep_(emax / ne),
                                                                                         emax_(emax), ecut_(ecut),
                                                                                         energy_fluence_(nThreads),
                                                                                         number_spectra_(nThreads),
                                                                                         energy_spectra_(nThreads)
{
...
void mcScoreSpectraFluence::dumpStatistic(ostream& os) const
{
   ...
   for (ir = 0; ir < nr_; ir++) {
      os << (ir + 0.5) * rstep_;
      for (ie = 0; ie < ne_; ie++)
         os << "\t" << (eSpectrumBin(ir, ie) * sf / (2 * ir + 1));
      os << endl;
   }
}
