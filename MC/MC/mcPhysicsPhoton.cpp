...
double mcPhysicsPhoton::DoInterruction(mcParticle* p, const mcMedium* med) const
{
   ...
   if (m->rayleigh && rng.rnd() < 1.0 - rayleighFactor) {
      ...
      double branch = rng.rnd(); // br1 = Pr(pair production)
      if (branch < m->br10_phot[iLogKE] + logKE * m->br11_phot[iLogKE])
         ProducePair(p, m); // br2 = Pr(pair production) + Pr(Compton)
...
void mcPhysicsPhoton::ProducePair(mcParticle* p, const mcMediumXE* pMedium)
{
   ...
   mcRng& rng = p->thread_->rng(); // Get the energies and the polar angles
   ...
   sinTheta2 = -minusSinTheta2; // Choose an arbitrary azimuthal angle
   double cosPhi, sinPhi;
   GetRandomPhi(rng.rnd(), &cosPhi, &sinPhi); // Choose randomly which particle is the negatron and which
   // is the positron
   int q_elec1, q_elec2;
...
void mcPhysicsPhoton::GetPairEnergies(mcRng& rng, const mcMediumXE* pMedium, double e_in, double* ke_elec1,
                                      double* ke_elec2)
{
   double e_elec2;
   if (e_in <= 2.1)
      ...
      int ascrn = A_BH;
      int cscrn = C_BH; // Loop until a value of r is accepted
      for (;;) {
         int iscrn; // Decide which part of the distribution to sample from and,
         // accordingly, which screening rejection function to use
         if (rng.rnd() >= pMedium->aOrC[idist]) {
            // Use A(delta) as the screening function
            iscrn = ascrn; // Sample from the subdistribution that is proportional
            // to 12 * (r-0.5)^2. By symmetry, one need only sample
            // r in the interval (0.0,0.5).
            double rnd0 = rng.rnd(), rnd1 = rng.rnd(), rnd2 = rng.rnd();
            double rndMax = MAX(rnd0, MAX(rnd1, rnd2));
            r = 0.5 * (1.0 - rndMax);
         } else {
            // Use C(delta) as the screening function
            iscrn = cscrn; // Sample from the subdistribution that is uniform
            r = 0.5 * rng.rnd();
         }
         if (r != 0.0) {
            ...
            if (del < pMedium->delLimit[idist]) {
               ...
               double screenLevel = (delta < 1.0)
                                       ? pMedium->screen0[iscrn] + delta * (
                                            pMedium->screen1[iscrn] + delta * pMedium->screen2[iscrn])
                                       : pMedium->screen0a[iscrn] + pMedium->screen1a[iscrn] * log(
                                            delta + pMedium->screen2a[iscrn]);
...
// 
void mcPhysicsPhoton::GetPairAngle(mcRng& rng, const mcMediumXE* pMedium, double eScaled_in, double ke_elec,
                                   double* cosTheta, double* sinTheta)
{
   ...
   if (pMedium->pairAngleOption == LOWEST_ORDER || (
      ...
      (*cosTheta) = (e_elec * cosTemp + p_elec) / (p_elec * cosTemp + e_elec);
      double sinSquaredTemp = (1.0 - cosTemp) * (1.0 + cosTemp);
      sinSquaredTemp = MAX(0, sinSquaredTemp);
      (*sinTheta) = EMASS * sqrt(sinSquaredTemp) / (p_elec * cosTemp + e_elec);
   } else if (pMedium->pairAngleOption == MOTZ_OLSEN_KOCH) {
      double e_elec = ke_elec + EMASS;
      double eScaled_elec = e_elec / EMASS;
      double zFactor = pMedium->zFactor_angDist; // Calculate some parameters for repeated use
      ...
      val1 *= val1; // Compute N_g (see page 5 of PIRS-0287R1)
      ...
      double N_g = 1.0 / (1.02 * MAX(g_ximin, g_xi1)); // Loop until a value of xi is accepted
      double theta, g_xi;
      do {
         double xi = rng.rnd();
...
double mcPhysicsPhoton::PairRejectionFunction(double x, double zFactor, double r, double oneOverR, double val1)
{
...
void mcPhysicsPhoton::ComptonScatter(mcParticle* p, const mcMediumXE* pMedium)
{
   ...
   mcRng& rng = p->thread_->rng(); // Get the energies and polar angles
   GetComptonSplit(rng, p->ke, &e_phot, &cosTheta_phot, &sinTheta_phot, &ke_elec, &cosTheta_elec, &sinTheta_elec);
   // Choose an arbitrary azimuthal angle
   double cosPhi, sinPhi;
   GetRandomPhi(rng.rnd(), &cosPhi, &sinPhi); // Instantiate a new particle and assign the appropriate charges,
   // energies, and directions
   mcParticle* pNextParticle = DuplicateParticle(p);
   if (e_phot < ke_elec) {
...
// 
void mcPhysicsPhoton::GetComptonSplit(mcRng& rng, double e_in, double* e_phot, double* cosTheta_phot,
                                      double* sinTheta_phot, double* ke_elec, double* cosTheta_elec,
                                      double* sinTheta_elec)
{
   ...
   double oneMinusR;
   double r, oneMinusCos, sinSquared; // Loop until an interaction is accepted
   do {
      if (alpha1 >= rng.rnd() * alphaSum) // Use 1/r part of distribution
   ...
   } while (rng.rnd() > 1.0 - r * sinSquared / (1.0 + r * r)); // Finish calculating scattering angle for photon
   (*sinTheta_phot) = sqrt(sinSquared);
   (*cosTheta_phot) = 1.0 - oneMinusCos; // Calculate energy and scattering angle for electron
   (*ke_elec) = e_in - (*e_phot);
...
{
   mcRng& rng = p->thread_->rng();
   p->plast = p->p; // Compute the deposited energy, the photoelectron energy, and
   ...
   if (ke_elec == 0) {
      ...
      p->ke = 0; // Note: The particle will be discarded at the start of the
...
void mcPhysicsPhoton::GetPhotoelectricEnergies(double rnum, const mcMediumXE* pMedium, double e_in, double* e_dep,
                                               double* ke_elec, double* e_fluor)
{
...
void mcPhysicsPhoton::ChangePhotoelectronDirection(mcRng& rng, double ke_elec, geomVector3D& u)
{
   ...
   double xi, sinSquared, cosTheta; // Loop until values of cos(theta) and sin^2(theta) are accepted
   if (betaOverAlpha <= 0.2)
      do {
      ...
      do {
         double x = 1.0 - 2.0 * rng.rnd();
         xi = gammaSquared * (1.0 + alpha * (sqrt(1.0 + betaOverAlpha * (2.0 * x + betaOverAlpha)) - 1.0));
...
void mcPhysicsPhoton::GetRayleighAngle(mcRng& rng, const mcMediumXE* pMedium, double e_phot, double* cosTheta,
                                       double* sinTheta)
{
   ...
   for (;;) {
      ...
      double qFactor = pMedium->raylQFactor0[iLogKE] + rnd0 * pMedium->raylQFactor1[iLogKE];
      double qSquared = qFactor * rayleighConstant;
      *cosTheta = 1.0 - qSquared * eFactor;
      if (ABS(*cosTheta) > 1.0)
         continue;
      cosSquared = SQUARE(*cosTheta);
      if (rng.rnd() <= 0.5 * (1.0 + cosSquared))
         break;
   }
...
{
   mcRng& rng = p->thread_->rng(); // Get the polar angle
   double cosTheta, sinTheta;
   GetRayleighAngle(rng, pMedium, p->ke, &cosTheta, &sinTheta); // Choose an arbitrary azimuthal angle
   double cosPhi, sinPhi;
   GetRandomPhi(rng.rnd(), &cosPhi, &sinPhi); // Set the new direction
