...
mcScoreTrack::mcScoreTrack(int nThreads, double R, double Z1, double Z2, double EMIN, bool doPhotons, bool doElectrons,
                           bool doPositrons) : mcScore(nullptr, nThreads), R_(R), Z1_(Z1), Z2_(Z2), EMIN_(EMIN),
                                               doPhotons_(doPhotons), doElectrons_(doElectrons),
                                               doPositrons_(doPositrons)
{
...
void mcScoreTrack::score(int iThread, mc_particle_t pt, const geomVector3D& p0, const geomVector3D& p1, double ke)
{
   if ((pt == MCP_PHOTON && !doPhotons_) || (pt == MCP_NEGATRON && !doElectrons_) || (
          pt == MCP_POSITRON && !doPositrons_) || ke < EMIN_)
      return;
   geomVector3D pp0 = p0;
   geomVector3D pp1 = p1;
...
void mcScoreTrack::dumpVRML(ostream& os) const
{
   ...
   for (ia = 0; ia < 4; ia++) {
      ...
      if (ia == 0)
         ...
         os << "            emissiveColor 1 0 0" << endl; //if(ia == 0)
      ...
      for (j = 0; j < static_cast<int>(t.size()); j++) {
         for (i = 0; i < static_cast<int>(t[j].size()); i++) {
            os << "                    " << t[j][i].x() << ' ' << t[j][i].y() << ' ' << t[j][i].z() << ", " << endl;
            i++;
            os << "                    " << t[j][i].x() << ' ' << t[j][i].y() << ' ' << t[j][i].z();
            if (i < static_cast<int>(t[j].size()) - 1)
               os << ", ";
            os << endl;
         }
      }
      ...
      int jcount = 0;
      for (j = 0; j < static_cast<int>(t.size()); j++) {
         for (i = 0; i < static_cast<int>(t[j].size()); i += 2, jcount += 2) {
