...
mcScoreAcceleratedBeam::mcScoreAcceleratedBeam(const char* module_name, int nThreads, enum mc_particle_t ptype, int ne,
                                               int nr, int naxial, int nthet, double emin, double emax, double rmax,
                                               double thetmax) : mcScore(module_name, nThreads), ptype_(ptype), ne_(ne),
                                                                 nr_(nr), nthet_(nthet), naxial_(naxial), emin_(emin),
                                                                 emax_(emax), rmax_(rmax), skipped_energy_(0)
{
   ...
   evr_.resize(nr_);
   for (i = 0; i < static_cast<int>(evr_.size()); i++)
      evr_[i].resize(ne_, 0);
   eva_.resize(naxial_);
   for (i = 0; i < static_cast<int>(eva_.size()); i++)
      eva_[i].resize(ne_, 0);
   tva_.resize(naxial_);
   for (i = 0; i < static_cast<int>(tva_.size()); i++)
      tva_[i].resize(nthet_, 0);
   wvxy_.resize(nr_ * 2);
   for (i = 0; i < static_cast<int>(wvxy_.size()); i++)
      wvxy_[i].resize(nr_ * 2, 0);
   wvvxvy_.resize(nthet_ * 2);
   for (i = 0; i < static_cast<int>(wvvxvy_.size()); i++)
      wvvxvy_[i].resize(nthet_ * 2, 0);
}
...
void mcScoreAcceleratedBeam::dumpStatistic(ostream& os) const
{
   ...
   for (i = 0; i < static_cast<int>(evr_.size()); i++) {
      os << (i + 0.5) * dr_;
      for (j = 0; j < static_cast<int>(evr_[0].size()); j++)
         os << "\t" << evr_[i][j];
      os << endl;
   }
   ...
   for (i = 0; i < static_cast<int>(eva_.size()); i++) {
      os << (i + 0.5) * daxial_ * 180 / PI;
      for (j = 0; j < static_cast<int>(eva_[0].size()); j++)
         os << "\t" << eva_[i][j];
      os << endl;
   }
   ...
   for (i = 0; i < static_cast<int>(tva_.size()); i++) {
      os << (i + 0.5) * daxial_ * 180 / PI;
      for (j = 0; j < static_cast<int>(tva_[0].size()); j++)
         os << "\t" << tva_[i][j] / (2 * j + 1);
      os << endl;
   }
   ...
   for (i = 0; i < static_cast<int>(wvxy_.size()); i++) {
      os << (i + 0.5 - nr_) * dr_;
      for (j = 0; j < static_cast<int>(wvxy_[0].size()); j++)
         os << "\t" << wvxy_[i][j];
      os << endl;
   }
   ...
   for (i = 0; i < static_cast<int>(wvvxvy_.size()); i++) {
      ...
      for (j = 0; j < static_cast<int>(wvvxvy_[0].size()); j++)
         os << "\t" << wvvxvy_[i][j];
      os << endl;
   }
}
