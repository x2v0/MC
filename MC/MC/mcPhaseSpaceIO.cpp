...
mcPhaseSpaceIO::mcPhaseSpaceIO() : open_mode_(NOT_OPEN), mode_(UNKNOWN), buf_pos(0), read_pos(0), file(nullptr)
{
...
void mcPhaseSpaceIO::reset()
{
   ...
   preamble_.ekin_max = static_cast<float>(0.);
...
void mcPhaseSpaceIO::fill()
{
   if (open_mode_ == READING) {
      ...
      // of records read
      if (mode_ == MODE2) {
         buf_pos = static_cast<unsigned>(fread(long_buffer, sizeof(record_long_t), BUFFER_SIZE, file));
         stripZLast();
      } else {
         buf_pos = static_cast<unsigned>(fread(short_buffer, sizeof(record_short_t), BUFFER_SIZE, file));
      }
      read_pos = 0; // If we've reached the EOF, rewind the stream
      if (feof(file)) {
...
void mcPhaseSpaceIO::open(const char* filename, open_mode_e open_mode)
{
   ...
   if (open_mode_ == WRITING)
      mode_ = MODE0; // Can write only MODE0 files
   else {
      // Read preamble
      char modekey[5];
      fread(modekey, sizeof(char), 5, file);
      fread(&preamble_, sizeof(preamble_t), 1, file);
      if (!strncmp(modekey, "MODE0", 5))
         mode_ = MODE0;
      else
         if (!strncmp(modekey, "MODE2", 5))
            mode_ = MODE2;
      char dummy[7];
      switch (mode_) {
         case MODE0:
...
{
   if (open_mode_ != READING)
      throw std::exception("File is not open for reading");
   if (read_pos >= buf_pos)
      fill();
   if (!buf_pos)
      fill();
   if (!buf_pos)
      throw std::exception("No records in file");
...
{
   if (open_mode_ != WRITING)
      throw std::exception("File is not open for writing");
   ...
   preamble_.n_incident_particles = float(preamble_.n_particles);
   if (preamble_.ekin_max < p.ke)
      preamble_.ekin_max = static_cast<float>(p.ke);
   record_short_t rec;
   rec.x_ = static_cast<float>(p.p.x());
   rec.y_ = static_cast<float>(p.p.y());
   rec.u_ = static_cast<float>(p.u.x());
   rec.v_ = static_cast<float>(p.u.y());
   rec.weight_times_sign_w_ = static_cast<float>(p.weight);
   if (p.u.z() < 0.)
      rec.weight_times_sign_w_ = -rec.weight_times_sign_w_;
   switch (p.t) {
      case MCP_PHOTON:
         rec.latch_ = PHOTON_LATCH;
         rec.energy_ = static_cast<float>(p.ke);
         preamble_.n_photons += 1;
         break;
      case MCP_POSITRON:
         rec.latch_ = POSITRON_LATCH;
         rec.energy_ = float(p.ke + EMASS);
         if (preamble_.e_min_electrons > float(p.ke + TWICE_EMASS))
            preamble_.e_min_electrons = float(p.ke + TWICE_EMASS);
         break;
      case MCP_NEGATRON:
         rec.latch_ = NEGATRON_LATCH;
         rec.energy_ = float(p.ke + EMASS);
         if (preamble_.e_min_electrons > float(p.ke))
            preamble_.e_min_electrons = float(p.ke);
         break;
      default:
