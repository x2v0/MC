...
mcScoreBeamFluence2::mcScoreBeamFluence2(const char* module_name, int nThreads, int nr, double rmax, int nr_s,
                                         double rmax_s, double H, int nz, double d_iz) : mcScore(module_name, nThreads),
                                                                                         h_(H), nz_(nz), d_iz_(d_iz)
{
   m_nr = nr;
   m_rmax = rmax;
   m_rstep = m_rmax / m_nr;
   ...
   for (it = 1; it < nThreads; it++) {
      ...
      for (iz = 1; iz < nz; iz++)
         m_intencity_s[it][iz] = m_intencity_s[it][iz - 1] + m_nr_s;
   }
}
...
{
   if (m_intencity_all)
      delete[] m_intencity_all;
   if (m_intencity_count_all)
      delete[] m_intencity_count_all;
   if (m_intencity)
      delete[] m_intencity;
   if (m_intencity_count)
      delete[] m_intencity_count;
   if (m_intencity_s) {
