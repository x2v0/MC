...
public:
   ...
   friend std::ostream& operator <<(std::ostream&, const mcSourceModelRadialPhsp&); /// <summary>
	...
	/// </summary>
   void* saveToMemory(int& size);                                                   /// <summary>
	/// Восстановления модели из образа памяти.
	/// </summary>
   void readFromMemory(void* buffer);                                               /// <summary>
	/// Создание самплеров из матрицы симуляции.
	/// </summary>
   void createFromDistribution(int isxray, int ne, int nr, int nt, int na, double emax, double r2max, double tmax,
                               double focus, const double* data);
...
protected:
   int isxray_; // Положение плоскости источника на оси системы
   double z0_;
   int ne_;
   int nr_;
   int nthet_;
   ...
   double rmax_;
   double thetmax_abs_;
   double thetmax_; // Служебные геометрические переменные
   double de_;
   double dr_;
   double dthet_abs_;
   double dthet_;
   double daxial_; // Веса линий фотонов
   double we117_;
   double we133_;
   double wexscale_; // Гистограммы для самплинга положения и углов
