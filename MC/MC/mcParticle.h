...
class mcScoreTrack;
class mcThread;
class mcTransport; // Типы частиц введены в качестве индексов объектов транспорта
...
public:
   ...
   double weight;          // Расстояние до следующего взаимодействия в единицах длин среднего пробега
   double mfps;            // Относительная плотность региона, в котором находится частица в данный момент
   double regDensityRatio; // Указатель на транспортный объект,
   // в котором частица находится в данный момент
   mcTransport* transport_; // Указатель на транспортный объект с минимальным расстоянием в случае возможности 
   // передачи управления модулю группы
   mcTransport* transportNearest_; // Место рождения частицы
   ...
   mcScoreTrack* trackScore_; // Указатель на объект, содержащий специфичные для потока данные.
   ...
   mcThread* thread_; // Объекты транспорта могут ставить печать в момент 
   // рождения частицы. Рассеянная частица означает ее рождение
   int regionBirth; // Объекты транспорта могут ставить печать в момент 
   // BeginTransport обозначая, что частица в них находилась.
   int regionFlags; // Тип пересекаемой поверхности (внешняя или внутрення) в embedded transport.
