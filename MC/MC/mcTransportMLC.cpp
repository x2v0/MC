...
mcTransportMLC::mcTransportMLC(const geomVector3D& orgn, const geomVector3D& vz, const geomVector3D& vx, double focus,
                               double r, double h, double w, double l, double fsx1, double fsx2, double fsy1,
                               double fsy2) : mcTransport(orgn, vz, vx), focus_(focus), r_(r), h_(h), w_(w), l_(l),
                                              fsx1_(fsx1), fsx2_(fsx2), fsy1_(fsy1), fsy2_(fsy2), pymin_(0, -w / 2, 0),
                                              pymax_(0, w / 2, 0), py1_(0, fsy1, 0), py2_(0, fsy2, 0)
{
   r2_ = r_ * r_;
...
double mcTransportMLC::getDistanceToBlockInside(const geomVector3D& p, const geomVector3D& u, double y1, double y2,
                                                double xx) const
{
...
double mcTransportMLC::getDistanceToMlcInsideFieldBlock(const geomVector3D& p, const geomVector3D& u, double x1,
                                                        double x2, double y1, double y2) const
{
...
double mcTransportMLC::getDistanceToMlcRectangleConeInside(const geomVector3D& p, const geomVector3D& u, double x1,
                                                           double x2, double y1, double y2) const
{
   double x = p.x();
   double ux = u.x();
...
bool mcTransportMLC::isInZPlaneHit(double x, double y) const
{
   ...
   if (y >= fsy1_ && y <= fsy2_) {
      if (x < (-l_ - fsx1_) || x > (l_ + fsx2_))
         return false;
      if (x > (-dx_ + fsx1_) && x < (dx_ + fsx2_))
         return false;
      return true;
   }
   return false;
...
void mcTransportMLC::dumpVRMLMLC(ostream& os) const
{
   ...
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[3] + ss[k], yy[2] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[2] + ss[k], yy[2] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[2] + ss[k], yy[1] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[3] + ss[k], yy[1] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[3] + ss[k], yy[0] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[4] - ss[k], yy[3] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[4] - ss[k], yy[2] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[5] - ss[k], yy[2] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[5] - ss[k], yy[1] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[4] - ss[k], yy[1] * (1 - dh * k / focus_), dh * k) * mttow_;
   for (k = 0; k < 7; k++)
      p[i++] = geomVector3D(xx[4] - ss[k], yy[0] * (1 - dh * k / focus_), dh * k) * mttow_;
   os << "    Transform {" << endl;
   ...
   for (k = 0; k < 6; k++)
      os << (lcount + 1 * 7 + k) << ", " << (lcount + 0 * 7 + k) << ", " << (lcount + 0 * 7 + k + 1) << ", " << (
         lcount + 1 * 7 + k + 1) << " -1," << endl;
   os << "                ";
   for (k = 0; k < 6; k++)
      os << (lcount + 3 * 7 + k) << ", " << (lcount + 2 * 7 + k) << ", " << (lcount + 2 * 7 + k + 1) << ", " << (
         lcount + 3 * 7 + k + 1) << " -1," << endl;
   os << "                ";
   for (k = 0; k < 6; k++)
      os << (lcount + 5 * 7 + k) << ", " << (lcount + 4 * 7 + k) << ", " << (lcount + 4 * 7 + k + 1) << ", " << (
         lcount + 5 * 7 + k + 1) << " -1," << endl;
   os << "                ";
   for (k = 0; k < 6; k++)
      os << (lcount + 6 * 7 + k) << ", " << (lcount + 7 * 7 + k) << ", " << (lcount + 7 * 7 + k + 1) << ", " << (
         lcount + 6 * 7 + k + 1) << " -1," << endl;
   os << "                ";
   for (k = 0; k < 6; k++)
      os << (lcount + 8 * 7 + k) << ", " << (lcount + 9 * 7 + k) << ", " << (lcount + 9 * 7 + k + 1) << ", " << (
         lcount + 8 * 7 + k + 1) << " -1," << endl;
   os << "                ";
   for (k = 0; k < 6; k++)
      os << (lcount + 10 * 7 + k) << ", " << (lcount + 11 * 7 + k) << ", " << (lcount + 11 * 7 + k + 1) << ", " << (
         lcount + 10 * 7 + k + 1) << " -1," << endl;
   os << "            ]" << endl;
   os << "        }" << endl;
   os << "      }" << endl;
   os << "    }" << endl;
