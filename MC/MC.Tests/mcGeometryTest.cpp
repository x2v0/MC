...
namespace MCTests
{
   ...
   public:
      TEST_METHOD(getDistanceToInfiniteCylinderInside)
      {
         ...
         Assert::AreEqual(sqrt(100.0 - 25.0), d, DBL_EPSILON, L"getDistanceToInfiniteCylinderInside failed",
                          LINE_INFO());
         ...
         Assert::AreEqual(sqrt((100.0 - 25.0) * 2), d, DBL_EPSILON * 10, L"getDistanceToInfiniteCylinderInside failed",
                          LINE_INFO());
      }
      ...
      TEST_METHOD(getDistanceToSphereOutside)
      {
         ...
         Assert::AreEqual(r * (2.0 - sqrt(0.75)), d, 10 * DBL_EPSILON, L"getDistanceToSphereOutside failed",
                          LINE_INFO());
         p(1) = -r * 0.5;
         p(2) = -r * 0.5;
         d = mcGeometry::getDistanceToSphereOutside(p, v, r);
         Assert::AreEqual(r * (2.0 - sqrt(0.5)), d, 10 * DBL_EPSILON, L"getDistanceToSphereOutside failed",
                          LINE_INFO());
      }
   };
}
