...
namespace MCTests
{
   ...
   public:
      ...
      {
         mcMedia media;
         media.addName("AIR700ICRU"); //media.initXEFromFile("../data/AcceleratorSimulator.pegs4dat");
         media.initXEFromFile(
            "C:/Users/GennadyGorlachev/Documents/GitHub/RoissVS/MC/data/AcceleratorSimulator.pegs4dat");
         mcThread thread;
         thread.setId(0);
         mcParticle p;
         ...
         auto t_c1 = new mcTransportCylinder(geomVector3D(0, 0, -20.0), geomVector3D(0, 0, 1), geomVector3D(1, 0, 0),
                                             5.0, 10);
         t_c1->setName("C1");
         t_c1->setMediaRef(&media);
         t_c1->setMediumMono("AIR700ICRU");
         auto t_c2 = new mcTransportCylinder(geomVector3D(0, 0, 10.0), geomVector3D(0, 0, 1), geomVector3D(1, 0, 0),
                                             5.0, 10);
         t_c2->setName("C2");
         ...
         for (i = 0; i < 10; i++)
            t_cglobal.beginTransport(p); // 2
         p.p.set(-30, 0, 0);
         for (i = 0; i < 10; i++)
            t_cglobal.beginTransportInside(p); // 3
         for (i = 0; i < 10; i++)
            t_c1->beginTransport(p); // 4
         p.p.set(0, 0, 0);
         for (i = 0; i < 10; i++)
            t_c2->beginTransport(p); // 5
         p.p.set(0, 0, 15);
         p.u.set(0, 0, -1);
         for (i = 0; i < 10; i++)
            t_c2->beginTransportInside(p); // 6
         p.p.set(-20, 0, -15);
         p.u.set(1, 0, 0);
         for (i = 0; i < 10; i++)
            t_cglobal.beginTransport(p); // 7
         p.p.set(20, 0, 15);
         p.u.set(-1, 0, 0);
         for (i = 0; i < 10; i++)
