...
namespace MCTests
{
   ...
   public:
      ...
      {
         auto score = createTestScore(); // 1
         score->ScorePoint(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_PHOTON, geomVector3D(0, 0, 10.1));
         Assert::AreEqual(1.0, score->Dose(0, 50), DBL_EPSILON * 10, L"ScorePoint failed", LINE_INFO()); // 2
         score->ScorePoint(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_PHOTON, geomVector3D(0, 2.0, 5.1));
         Assert::AreEqual(1.0, score->Dose(21, 25), DBL_EPSILON * 10, L"ScorePoint failed", LINE_INFO()); // 3
         score->ScorePoint(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_PHOTON, geomVector3D(5.0, 0, 20.1));
      ...
      {
         int i, j;
         auto score = createTestScore(); // 1 - вдоль центральной оси на длину скоринга
         ...
         score->ScoreLine(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_PHOTON,
                          geomVector3D(0, 5.0 * 90.1 / 80.0, 20.1), geomVector3D(0, 0, 20.1));
         Assert::AreEqual(2.0 / 50 + 2.0 / 175, score->Dose(0, 100), DBL_EPSILON * 100, L"ScoreLine failed",
                          LINE_INFO());
         for (i = 1; i < 50; i++)
         ...
         score->ScoreLine(2.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_PHOTON,
                          geomVector3D(5.0 * 105.0 / 80.0, 0, 35.0), geomVector3D(0, 5.0 * 70.0 / 80.0, 0));
         double sum = 0;
         for (i = 0; i < 50; i++) {
            for (j = 0; j < 175; j++)
               sum += score->Dose(i, j);
   ...
   private:
      ...
      {
         return std::make_shared<mcScoreConicalRZ>("mcScoreConicalRZ test", 1, 50, 175, 5.0, 0.0, 35.0, 10.0, 80.0);
      }
   };
}
