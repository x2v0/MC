...
namespace MCTests
{
   ...
   public:
      TEST_METHOD(ScorePoint)
      {
         ...
         score->ScorePoint(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_NEGATRON,
                           geomVector3D(0.0001, 1, 100));
         Assert::AreEqual(1.0, score->Dose(0, 8), DBL_EPSILON * 10, L"ScorePoint failed", LINE_INFO()); // 2
         score->ScorePoint(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_NEGATRON,
                           geomVector3D(-0.0001, -1, -100));
         Assert::AreEqual(1.0, score->Dose(17, 26), DBL_EPSILON * 10, L"ScorePoint failed", LINE_INFO()); // 3
         score->ScorePoint(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_NEGATRON, geomVector3D(100.0, 1, 1));
      ...
      TEST_METHOD(ScoreLine)
      {
         ...
         v1.normalize();
         v2.normalize();
         score->ScoreLine(100.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_NEGATRON, v1 * 102, v2 * 102);
         ...
         v1.normalize();
         v2.normalize();
         score->ScoreLine(1.0, 0, *static_cast<mcRegionReference*>(nullptr), MCP_NEGATRON, v1 * 101, v2 * 101);
   ...
   private:
      ...
      {
         return std::make_shared<mcScoreSphereMatrix>("mcScoreSphereMatrix test", 1, 18, 36, 99, 101);
      }
   };
}
