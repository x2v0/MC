...
namespace MCTests
{
   ...
   public:
      TEST_METHOD(getDistanceOutside)
      {
         ...
         mcParticle p;
         std::wstring err(L"mcTransportMLCTest::getDistanceOutside failed"); // 1 (0)
         p.p = geomVector3D(0, 0, 5);
         p.u = geomVector3D(1, 0, 0);
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(5.0, d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 2 (0)
         ...
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(2.5 * sqrt(5.0), d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 3 (0)
         p.p = geomVector3D(0, 0, 10.0);
         p.u = geomVector3D(10.0 * (1 - sqrt(1 - 0.0625)), 7.5, -2.5);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(7.9, d, 0.1, err.c_str(), LINE_INFO()); // 4 (0)
         p.p = geomVector3D(0, 0, 5);
         ...
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(6.0, d, 0.1, err.c_str(), LINE_INFO()); // 5 (0)
         ...
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(DBL_MAX, d, 0, err.c_str(), LINE_INFO()); // 6 (1)
         p.p = geomVector3D(36, 11, 5);
         p.u = geomVector3D(-1, -11, 0);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(11.05, d, 0.05, err.c_str(), LINE_INFO()); // 7 (1)
         p.u = geomVector3D(-3, -6, 0);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(3.25 * sqrt(5), d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 8 (1)
         p.u = geomVector3D(-6, -3, 0);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(3.0 * sqrt(5), d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 9 (1)
         ...
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(sqrt(1602.25), d, 0.05, err.c_str(), LINE_INFO()); // 10 (2)
         p.p = geomVector3D(36, 0, 11);
         p.u = geomVector3D(-3, 0, -1);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(sqrt(10.0), d, 0.05, err.c_str(), LINE_INFO()); // 11 (2)
         ...
         //
         auto transportAssym = createTestTransportAssym(); // 12 (3)
         p.p = geomVector3D(-40, -11, 5);
         p.u = geomVector3D(10, 3, 0);
         p.u.normalize();
         d = transportAssym->getDistanceOutside(p);
         Assert::AreEqual(sqrt(109), d, 0.05, err.c_str(), LINE_INFO()); // 13 (3)
         p.u = geomVector3D(15, 10, 0);
         p.u.normalize();
         d = transportAssym->getDistanceOutside(p);
         Assert::AreEqual(sqrt(325), d, 0.05, err.c_str(), LINE_INFO()); // 14 (3)
         ...
         p.u.normalize();
         d = transportAssym->getDistanceOutside(p);
         Assert::AreEqual(sqrt(100 + 17 * 17), d, 0.05, err.c_str(), LINE_INFO()); // 15 (4)
      ...
      TEST_METHOD(getDistanceInside)
      {
         ...
         auto transport = createTestTransport();
         mcParticle p;
         std::wstring err(L"mcTransportMLCTest::getDistanceInside failed"); // 1 (0)
         ...
         p.u.normalize();
         d = transport->getDistanceInside(p);
         Assert::AreEqual(25.0, d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 3 (0)
         ...
         p.u.normalize();
         d = transport->getDistanceInside(p);
         Assert::AreEqual(5.0 * 0.9 * sqrt(2), d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 4 (0)
         ...
         p.u.normalize();
         d = transport->getDistanceInside(p);
         Assert::AreEqual(sqrt(101) * 0.45, d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 5 (1)
         p.p = geomVector3D(10, 0, 5);
         p.u = geomVector3D(-1, 0, 0);
         p.u.normalize();
         d = transport->getDistanceInside(p);
         Assert::AreEqual(5.0, d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 6 (1)
         p.u = geomVector3D(1, 0, 0);
         p.u.normalize();
         d = transport->getDistanceInside(p);
         Assert::AreEqual(25.0, d, DBL_EPSILON * 10, err.c_str(), LINE_INFO()); // 7 (1)
   ...
   private:
      static std::shared_ptr<mcTransportMLC> createTestTransport()
      {
         ...
         double y1 = -5.0;
         double y2 = 5.0;
         auto t = std::make_shared<mcTransportMLC>(geomVector3D(0, 0, 0), geomVector3D(0, 0, 1), geomVector3D(1, 0, 0),
                                                   focus, radius, height, width, length, x1, x2, y1, y2);
         return t;
      }
