...
namespace MCTests
{
   ...
   public:
      TEST_METHOD(getDistanceOutside)
      {
         ...
         Assert::AreEqual(1.0 / p.u.z(), d, DBL_EPSILON * 10, L"getDistanceOutside failed", LINE_INFO()); // 6 (3)
         p.p = geomVector3D(0, -2, -4);
         p.u = geomVector3D(0, -2, 1);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(DBL_MAX, d, DBL_EPSILON * 10, L"getDistanceOutside failed", LINE_INFO()); // 7 (4)
         p.p = geomVector3D(-4, 0, 7);
         p.u = geomVector3D(4, 0, -3);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(-2.0 / p.u.z(), d, DBL_EPSILON * 10, L"getDistanceOutside failed", LINE_INFO()); // 8 (4)
         p.u = geomVector3D(0.5, 0, -3);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(2.8, d, 0.2, L"getDistanceOutside failed", LINE_INFO()); // 9 (4)
         p.u = geomVector3D(-1, 0, -3);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(DBL_MAX, d, DBL_EPSILON * 10, L"getDistanceOutside failed", LINE_INFO()); // 10 (5)
         p.p = geomVector3D(0, -7, 4.5);
         p.u = geomVector3D(0, 7, -0.5);
         p.u.normalize();
         d = transport->getDistanceOutside(p);
         Assert::AreEqual(3.5, d, 0.2, L"getDistanceOutside failed", LINE_INFO()); // 11 (5)
      ...
      TEST_METHOD(getDistanceInside)
      {
         ...
         Assert::AreEqual(4.0, d, DBL_EPSILON * 10, L"getDistanceInside failed", LINE_INFO()); // 2 (1)
         p.u = geomVector3D(0, 2.000000001, 4);
         p.u.normalize();
         d = transport->getDistanceInside(p);
         Assert::AreEqual(sqrt(20), d, DBL_EPSILON * 10, L"getDistanceInside failed", LINE_INFO()); // 3 (2)
         p.p = geomVector3D(2, 0, 1);
         p.u = geomVector3D(1, 0, 0);
         d = transport->getDistanceInside(p);
         Assert::AreEqual(2.0, d, DBL_EPSILON * 10, L"getDistanceInside failed", LINE_INFO()); // 4 (2)
         p.u = geomVector3D(2, 0, 4);
         p.u.normalize();
         d = transport->getDistanceInside(p);
   ...
   private:
      static std::shared_ptr<mcETransportConvexPolygonCircle> createTestTransport()
      {
         ...
         poly_z.push_back(5);
         poly_r.push_back(2);
         return std::make_shared<mcETransportConvexPolygonCircle>(geomVector3D(0, 0, 0), geomVector3D(0, 0, 1),
                                                                  geomVector3D(1, 0, 0), poly_z, poly_r);
      }
      ...
      static std::shared_ptr<mcETransportConvexPolygonCircle> createTestTransport2()
      {
         ...
         std::vector<double> poly_r;
         poly_z.push_back(-22);
         poly_r.push_back(5.5);
         poly_z.push_back(-2);
         poly_r.push_back(9.5);
         poly_z.push_back(5.3);
         poly_r.push_back(9.5);
         poly_z.push_back(9.8);
         poly_r.push_back(5);
         return std::make_shared<mcETransportConvexPolygonCircle>(geomVector3D(0, 0, 0), geomVector3D(0, 0, 1),
                                                                  geomVector3D(1, 0, 0), poly_z, poly_r);
      }
