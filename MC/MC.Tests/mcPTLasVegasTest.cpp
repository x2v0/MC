...
namespace MCTests
{
   ...
   public:
      TEST_METHOD(getDistanceOutside)
      {
         ...
         Assert::AreEqual(10, d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 2
         p.p = geomVector3D(transport.A() / 2 + 1, 0, 0);
         p.u = geomVector3D(-sqrt(0.5), 0, sqrt(0.5));
         d = transport.getDistanceOutside(p);
         Assert::AreEqual(sqrt(2.0), d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 3
         p.p = geomVector3D(-transport.A() / 2 - 1, 0, 0);
         p.u = geomVector3D(sqrt(0.5), 0, sqrt(0.5));
         d = transport.getDistanceOutside(p);
         Assert::AreEqual(sqrt(2.0), d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 4
         p.p = geomVector3D((x0 + x1) * 0.5, (y0 + y1) * 0.5, -1);
         p.u = geomVector3D(0, 0, 1);
         d = transport.getDistanceOutside(p);
         Assert::AreEqual(1, d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 5
         p.p = geomVector3D(x0 - d0 - h0, y0, -d0 / 2);
         p.u = geomVector3D(sqrt(0.5), 0, sqrt(0.5));
         d = transport.getDistanceOutside(p);
         Assert::AreEqual(d0 * sqrt(0.5), d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 6
         p.p = geomVector3D(x0, y0, 0);
         p.u = geomVector3D(0, 0, 1);
         d = transport.getDistanceOutside(p);
         Assert::AreEqual(h0, d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 7
         p.p = geomVector3D(x0 + d0 / 2 - 1.5 * h0, y0, -h0);
         p.u = geomVector3D(sqrt(0.5), 0, sqrt(0.5));
         d = transport.getDistanceOutside(p);
         Assert::AreEqual(1.5 * h0 * sqrt(2.0), d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 8
         p.p = geomVector3D(x0, y0, h0);
         p.u = geomVector3D(0, 0, 1);
         d = transport.getDistanceOutside(p);
         Assert::AreEqual(0, d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 9
         p.p = geomVector3D(x0, y0, h0);
         p.u = geomVector3D(0, 0, -1);
         d = transport.getDistanceOutside(p);
      ...
      TEST_METHOD(getDistanceInside)
      {
         ...
         Assert::AreEqual((transport.Z() - 1) * sqrt(2.0), d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 2
         ...
         p.u = geomVector3D(-sqrt(0.5), 0, -sqrt(0.5));
         d = transport.getDistanceInside(p);
         Assert::AreEqual(sqrt(2.0) * h0, d, DBL_EPSILON * 10, errmsg, LINE_INFO()); // 5
