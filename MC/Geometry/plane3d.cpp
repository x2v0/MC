...
geomPlane3D::geomPlane3D(const geomPlane3D& p) : p_(p.getPoint()), n_(p.getNormal()), xv_(p.getXAxis()),
                                                 mRtoP_(p.getRtoPMatrix()), mPtoR_(p.getPtoRMatrix()) {}

void geomPlane3D::set(const geomVector3D& p, const geomVector3D& n, const geomVector3D& xv)
{
   /*
   // HACK !!!
...
geomVector3D geomPlane3D::crossByLine(const geomVector3D& p0, const geomVector3D& p1) const
{
...
double geomPlane3D::getPlanePosition() const
{
   return p_ * n_;
}

bool geomPlane3D::crossByEdge(const geomVector3D& p0, const geomVector3D& p1, double& x, double& y) const
{
   double h0 = (p0 - p_) * n_, h1 = (p1 - p_) * n_;
   if (h0 * h1 > 0)
      return false;
...
double geomPlane3D::nearestDistance(const geomVector3D& p) const
{
   return fabs((p - p_) * n_);
}
