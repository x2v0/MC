...
public:
   ...
   geomPlane3D(const geomVector3D& p, const geomVector3D& n, const geomVector3D& xv)
   {
      set(p, n, xv);
   }
   ...
   const geomVector3D& getPoint() const
   {
      return p_;
   }

   const geomVector3D& getNormal() const
   {
      return n_;
   }

   const geomVector3D& getXAxis() const
   {
      return xv_;
   }
   ...
   bool isAxial() const
   {
      return n_.x() == 0 && n_.y() == 0;
   }

   void setPoint(const geomVector3D& p)
   {
      p_ = p;
   }

   void setNormal(const geomVector3D& n)
   {
      n_ = n;
   }

   void setXAxis(const geomVector3D& xv)
   {
      xv_ = xv;
   }
   ...
   const geomMatrix3D& getRtoPMatrix() const
   {
      return mRtoP_;
   }

   const geomMatrix3D& getPtoRMatrix() const
   {
      return mPtoR_;
   } // Utilities
