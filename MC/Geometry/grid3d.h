...
public:
   ...
   unsigned numCol() const
   {
      return nx_;
   }

   unsigned numRow() const
   {
      return ny_;
   }

   double pixelSX() const
   {
      return psx_;
   }

   double pixelSY() const
   {
      return psy_;
   }

   geomVector3D getPointByIndex(int, int) const;

   void getPointByIndex(int i, int j, double& x, double& y) const;

   void setGrid(int nx, int ny, double psx, double psy);

   void setResolution(double x, double y); // Интелектуальная установка сетки при которой обязательно присутствуют
   ...
   void setAdjustedResolution(double x, double y);

   void setSize(int nx, int ny); // Индексы по заданным физическим координатам,
   ...
   int getRowIndex(double y) const;

   double getCol(unsigned i) const;

   double getRow(unsigned j) const;
