...
geomMatrix3D::geomMatrix3D(double a00, double a01, double a02, double a03, double a10, double a11, double a12,
...
void geomMatrix3D::makeInverse()
{
   ...
   for (i = 0; i < 4; i++) {
      // select pivot row
      pivot_to = i;
      scp_max = fabs(s[i][i] / scp[i]); // find out which row should be on top
      for (p = i + 1; p < 4; p++)
      ...
      double mji; // perform gaussian elimination
      for (j = i + 1; j < 4; j++) {
         mji = s[j][i] / s[i][i];
         s[j][i] = 0.0;
         for (jj = i + 1; jj < 8; jj++)
...
geomMatrix3D geomMatrix3D::BuildFromAxis(const geomVector3D& ax, const geomVector3D& ay, const geomVector3D& az)
{
   return geomMatrix3D(ax.x(), ay.x(), az.x(), 0, ax.y(), ay.y(), az.y(), 0, ax.z(), ay.z(), az.z(), 0, 0, 0, 0, 1);
}
