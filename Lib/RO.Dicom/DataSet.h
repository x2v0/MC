...
namespace RO
{
   namespace Dicom
   {
      class Element; // DataSet format is described in the standard:
      ...
      public:
         ...
         unsigned int NElementGroups() const
         {
            return static_cast<unsigned int>(elements_.size());
         }

         std::shared_ptr<std::map<UINT32, std::shared_ptr<Element>>> Elements(unsigned int idx = 0)
         {
         ...
         void AddElementDateTime(UINT32 tag, UINT16 year, UINT16 month, UINT16 day, UINT16 hour, UINT16 min, UINT16 sec,
                                 UINT32 tick, unsigned int idx = 0);
