...
void DataSet::AddElementDateTime(UINT32 tag, UINT16 year, UINT16 month, UINT16 day, UINT16 hour, UINT16 min, UINT16 sec,
                                 UINT32 tick, unsigned int idx)
{
   auto ditem = DicomDictionary::Item(tag);
   auto v = ElementFactory::MakeElement(tag, ditem->Vr);
...
void DataSet::Read(std::istream& is, unsigned int maxlen)
{
   ...
   while (true) {
      ...
      if (nextValue == ItemDelimitationItem) {
         ...
         if (nextValue == Item) // begin new item of undefined length
         {
            nextValue = ReadUINT32LE(is);
         ...
         elements_.push_back(std::make_shared<std::map<UINT32, std::shared_ptr<Element>>>());
         idx = static_cast<unsigned int>(elements_.size()) - 1;
#if DICOM_TRACE > 0
			wchar_t txt[64];
...
void DataSet::Write(std::ostream& os, TransferSyntaxType ts)
{
   ...
   for each (auto elements in elements_) {
      for_each(elements->begin(), elements->end(), [this, &os](std::pair<UINT32, std::shared_ptr<Element>> x)
      {
         WriteTag(os, x.first);
         if (ts_->ExplicitVr)
