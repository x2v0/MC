...
void DicomObject::ReadFromStream(std::istream& is, bool skipPixels)
{
   ...
   dset->Read(is); //MetaInfo->RemoveFromDataSet(*dset.get());
   Dset = dset;
}
...
void DicomObject::ReadFromFile(std::string fname, bool skipPixels)
{
   ...
   if (is.fail())
      throw std::exception(("File:\"" + fname + "\" not found").c_str()); // Skip preamble
   is.seekg(128);
   char prefix[4];
   is.read(prefix, 4);
...
void DicomObject::ReadFromBuffer(const char* data, unsigned int len)
{
   ...
   if (is.fail())
      throw std::exception("Can not create stream from memory buffer"); // Skip preamble
   is.seekg(128);
   char prefix[4];
   is.read(prefix, 4);
   if (std::string(prefix, 4) != "DICM")
...
void DicomObject::WtiteToStream(std::ostream& os)
{
   ...
   std::string s0(128, static_cast<char>(0));
...
void* DicomObject::PixelData()
{
   ...
   if (pe == nullptr)
      return nullptr;
   const char* data = pe->GetData(); //return static_cast<void*>(data);
   return (void*)(data);
}
