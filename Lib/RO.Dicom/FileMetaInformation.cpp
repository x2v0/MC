...
FileMetaInformation::FileMetaInformation() {} //FileMetaInformation::FileMetaInformation(DataSet& data)
//{
...
void FileMetaInformation::ReadFromStream(std::istream& is)
{
   ...
   while (true) {
      ...
      if (is.fail())
         throw std::exception(errmsg); //auto pos = is.tellg();
...
std::shared_ptr<FileMetaInformation> FileMetaInformation::CreateDefault()
{
   ...
   DataSet& metaelements = metaset->Elements(); //metaelements.AddElementString(MakeTag(0x0002, 0x0000), "");
   ...
   metaelements.AddElementString(TransferSyntaxUID, ExplicitVRLittleEndian);
