...
short GetTwoStringsFromLine(const string& line, string& s1, string& s2, const char* mask)
{
   s1.erase();
   s2.erase();
   int index = 0, len = static_cast<int>(line.size());
   if (len == 0)
      return 0;
   string mStr = (mask == nullptr) ? " \t,;=:)(\\" : mask; // skip all delimeters in the beginning of line
   while (index < len) {
   ...
   index = static_cast<int>(newline.find_first_of(mStr));
   if (index == string::npos) // no sublines (only one line)
      s1 = line;
   else {
      s1 = string(newline, 0, index); // skip all delimeters
      len = static_cast<int>(newline.size());
      while (index < len) {
   ...
   return static_cast<short>(s1.size());
}
...
std::string remove_chars(const std::string& s, const std::string& chars)
{
   ...
   auto it = std::copy_if(s.begin(), s.end(), result.begin(), [&chars](const char& c)
   {
...
std::string remove_non_alnum_chars(const std::string& s, const std::string& chars)
{
   std::locale loc;
   std::string result(s.length(), 0);
   auto it = std::copy_if(s.begin(), s.end(), result.begin(), [&chars, loc](const char& c)
   {
      return std::isalnum(c, loc) || chars.find(c) == std::string::npos;
   });
   result.resize(std::distance(result.begin(), it));
   return result;
} //
...
{
   throw std::exception(unsupported_msg_);
}

void Element::SetString(const std::string& s)
{
   throw std::exception(unsupported_msg_);
}

std::vector<double> Element::GetDouble() const
{
   throw std::exception(unsupported_msg_);
}

void Element::SetDouble(std::vector<double>)
{
   throw std::exception(unsupported_msg_);
}

std::vector<int> Element::GetInt() const
{
   throw std::exception(unsupported_msg_);
}

void Element::SetInt(std::vector<int>)
{
   throw std::exception(unsupported_msg_);
}

std::shared_ptr<DataSet> Element::GetDataSet() const
{
   throw std::exception(unsupported_msg_);
}

void Element::SetDataSet(std::shared_ptr<DataSet>)
{
   throw std::exception(unsupported_msg_);
}

void Element::GetDate(UINT16& year, UINT16& month, UINT16& day) const
{
   throw std::exception(unsupported_msg_);
}

void Element::GetTime(UINT16& hour, UINT16& min, UINT16& sec, UINT32& tick) const
{
   throw std::exception(unsupported_msg_);
}

void Element::GetDateTime(UINT16& year, UINT16& month, UINT16& day, UINT16& hour, UINT16& min, UINT16& sec,
                          UINT32& tick) const
{
   throw std::exception(unsupported_msg_);
}

void Element::SetDate(UINT16 year, UINT16 month, UINT16 day)
{
   throw std::exception(unsupported_msg_);
}

void Element::SetTime(UINT16 hour, UINT16 min, UINT16 sec, UINT32 tick)
{
   throw std::exception(unsupported_msg_);
}

void Element::SetDateTime(UINT16 year, UINT16 month, UINT16 day, UINT16 hour, UINT16 min, UINT16 sec, UINT32 tick)
{
   throw std::exception(unsupported_msg_);
}

void Element::ReadTag(std::istream& is)
{
   throw std::exception(unsupported_msg_);
}

void Element::ReadVr(std::istream& is)
{
   throw std::exception(unsupported_msg_);
}

long Element::ReadLength(std::istream& is)
{
   throw std::exception(unsupported_msg_);
}

void Element::WriteTag(std::ostream& os)
{
   throw std::exception(unsupported_msg_);
}

void Element::WriteVr(std::ostream& os)
{
   throw std::exception(unsupported_msg_);
}

void Element::WriteLength(std::ostream& os, long lengts)
{
   throw std::exception(unsupported_msg_);
}
...
{
   wchar_t txt[64];
   swprintf_s(txt, 64, L"%X\t%X\t%i\t", TagGroup(tag_), TagElement(tag_), static_cast<int>(vr_));
   return txt;
}
...
{
   if (ts->ExplicitVr)
      WriteUINT16(os, static_cast<UINT16>(data_.size()));
   else
      WriteUINT32(os, static_cast<UINT16>(data_.size()));
   os << data_;
}

std::string ElementString::GetString() const
{
   return data_;
}

void ElementString::SetString(const std::string& s)
{
   data_ = s;
}
...
std::vector<int> ElementString::GetInt() const
{
   ...
   for (;;) {
      if (!GetTwoStringsFromLine(l, l1, l2, nullptr))
         break;
...
std::string ElementOtherString::GetString() const
{
   return std::string(data_, len_);
}

void ElementOtherString::SetString(const std::string& s)
{
   SetData(&s[0], static_cast<int>(s.length()));
}
...
void ElementOtherString::Read(std::istream& is, UINT32 len, TransferSyntaxType ts)
{
   ...
   if (olen == 0xffffffff) {
      ...
      while (true) {
         ...
         if (is.fail())
            throw std::exception(stream_err_msg_);
         len_ += 4;
      }
   } else {
      if (data_ != nullptr)
...
{
   if (ts->ExplicitVr)
      WriteUINT16(os, static_cast<UINT16>(data_.size() * sizeof(int)));
   else
      WriteUINT32(os, static_cast<UINT16>(data_.size() * sizeof(int)));
   for (auto&& v : data_) {
      WriteUINT32LE(os, v);
   }
}

std::vector<int> ElementAT::GetInt() const
{
   return data_;
}

void ElementAT::SetInt(std::vector<int> val)
{
   data_ = val;
}
...
// DT
//
void ElementDT::GetDateTime(UINT16& year, UINT16& month, UINT16& day, UINT16& hour, UINT16& min, UINT16& sec,
                            UINT32& tick) const
{
   year = static_cast<UINT16>(atoi(data_.substr(0, 4).c_str()));
   month = static_cast<UINT16>(atoi(data_.substr(4, 2).c_str()));
...
{
   if (ts->ExplicitVr)
      WriteUINT16(os, static_cast<UINT16>(data_.size() * sizeof(float)));
   else
      WriteUINT32(os, static_cast<UINT16>(data_.size() * sizeof(float)));
   for (auto&& v : data_) {
...
std::vector<double> ElementFL::GetDouble() const
{
   return data_;
}

void ElementFL::SetDouble(std::vector<double> val)
{
   data_ = val;
}

std::wstring ElementFL::ToWString() const
{
   std::wstring ws = Element::ToWString();
...
{
   if (ts->ExplicitVr)
      WriteUINT16(os, static_cast<UINT16>(data_.size() * sizeof(double)));
   else
      WriteUINT32(os, static_cast<UINT16>(data_.size() * sizeof(double)));
   for (auto v : data_) {
      os.write(reinterpret_cast<char*>(&v), sizeof(double));
   }
}

std::vector<double> ElementFD::GetDouble() const
{
   return data_;
}

void ElementFD::SetDouble(std::vector<double> val)
{
   data_ = val;
}

std::wstring ElementFD::ToWString() const
{
   std::wstring ws = Element::ToWString();
   for each (double v in data_) {
      wchar_t txt[32];
...
std::vector<int> ElementSL::GetInt() const
{
   return data_;
}

void ElementSL::SetInt(std::vector<int> val)
{
   data_ = val;
}

std::wstring ElementSL::ToWString() const
{
   std::wstring ws = Element::ToWString();
   for each (int v in data_) {
      wchar_t txt[32];
...
{
   if (ts->ExplicitVr)
      WriteUINT16(os, static_cast<UINT16>(data_.size() * sizeof(INT16)));
   else
      WriteUINT32(os, static_cast<UINT16>(data_.size() * sizeof(INT16)));
   for (auto v : data_) {
      os.write(reinterpret_cast<char*>(&v), sizeof(INT16));
   }
}

std::vector<int> ElementSS::GetInt() const
{
   return data_;
}

void ElementSS::SetInt(std::vector<int> val)
{
   data_ = val;
}

std::wstring ElementSS::ToWString() const
{
   std::wstring ws = Element::ToWString();
   for each (int v in data_) {
      wchar_t txt[32];
...
{
   if (ts->ExplicitVr)
      WriteUINT16(os, static_cast<UINT16>(data_.size() * sizeof(UINT32)));
   else
      WriteUINT32(os, static_cast<UINT16>(data_.size() * sizeof(UINT32)));
   for (auto v : data_) {
      os.write(reinterpret_cast<char*>(&v), sizeof(UINT32));
   }
}

std::vector<int> ElementUL::GetInt() const
{
   return data_;
}

void ElementUL::SetInt(std::vector<int> val)
{
   data_ = val;
}

std::wstring ElementUL::ToWString() const
{
   std::wstring ws = Element::ToWString();
   for each (int v in data_) {
      wchar_t txt[32];
...
std::vector<int> ElementUS::GetInt() const
{
   return data_;
}

void ElementUS::SetInt(std::vector<int> val)
{
   data_ = val;
}

std::wstring ElementUS::ToWString() const
{
   std::wstring ws = Element::ToWString();
   for each (int v in data_) {
      wchar_t txt[32];
...
void ElementSQ::Write(std::ostream& os, TransferSyntaxType ts) const
{
   if (ts->ExplicitVr) {
      ...
      WriteUINT32LE(os, Item);       // item begin delimeter
   ...
   data_->Write(os, ts);
   WriteUINT32LE(os, ItemDelimitationItem);
   WriteUINT32LE(os, 0x00000000);
   WriteUINT32LE(os, ItemDelimitationItem);
   WriteUINT32LE(os, 0x00000000);
}

std::shared_ptr<DataSet> ElementSQ::GetDataSet() const
{
   return data_;
}

void ElementSQ::SetDataSet(std::shared_ptr<DataSet> ds)
{
   data_ = ds;
}
