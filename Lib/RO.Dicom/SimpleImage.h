...
namespace RO
{
   namespace Dicom
   {
      ...
      public:
         ...
         ~SimpleImage(void); // S E T
         // Модальность изображения (RTIMAGE для DRR)
         void SetModality(const char* modality); // Размер матрицы изображения
         ...
         void SetImageCorner(double x0, double y0)
         {
            x0_ = x0;
            y0_ = y0;
         ...
         void SetPixelData(unsigned short* data, double rs = 1.0, double ri = 0.0); // G E T
         unsigned short* GetPixelData()
         {
            return pixels_;
         }
         ...
         void ReadFromFile(const char* fname); // Запись данных включая мета-данные
