...
{
   if (_wcsicmp(st, L"photon") == 0 || _wcsicmp(st, L"c60") == 0 || _wcsicmp(st, L"c60_sphere") == 0 || _wcsicmp(
          st, L"phsp_photon") == 0)
      return MCP_PHOTON;
   if (_wcsicmp(st, L"electron") == 0)
      return MCP_NEGATRON;
   if (_wcsicmp(st, L"positron") == 0)
      return MCP_POSITRON;
...
enum spectrum_distr_t GeometryParser::convert_spec_type(const wchar_t* st)
{
   ...
   if (_wcsicmp(st, L"triangle") == 0)
      return SPECTRUM_TRIANGLE;
   if (_wcsicmp(st, L"prism") == 0)
...
mcTransport* GeometryParser::ParseTransport(const XPRNode& geometry, const mcMedia* media, int nThreads)
{
   ...
   bool skipInit = false;
   mcTransport* t = nullptr; // Общие для всех модулей параметры, которые должны быть получены в результате парсинга
   wstring geomType;
   wstring geomName;
   wstring geomMedium;
   ...
   double x0, y0, z0;
   double vx, vy, vz;
   double xx, xy, xz; // Специфичные для модуля параметры.
   ...
   int nc = 0;
   int nsplit = 0;
   mc_particle_t ptype = MCP_PHOTON;
   ...
   for (auto node : geometry.Nodes) {
      if (_wcsicmp(node.Name.c_str(), L"type") == 0)
         ...
         densityRatio = _wtof(node.Text.c_str()); // Transport Cutoff
   ...
   if (_wcsicmp(geomType.c_str(), L"cylinder") == 0) {
      ...
      static_cast<mcTransportJawPairRounded*>(t)->setFS(x1, x2);
      ...
      t = new mcTransport(origin, normal, xaxis); // Ищем и парсим вложенные модули
      for (auto node : geometry.Nodes) {
         if (_wcsicmp(node.Name.c_str(), L"module") == 0) {
            mcTransport* region = ParseTransport(node, media, nThreads);
      ...
      mcTransport* eprev = nullptr; // Ищем и парсим вложенные модули
      for (auto node : geometry.Nodes) {
         if (_wcsicmp(node.Name.c_str(), L"module") == 0) {
            mcTransport* eobj = ParseTransport(node, media, nThreads);
            if (eobj == nullptr)
      ...
      t = new mcTransportEmbeddedGroup(origin, normal, xaxis); // Ищем и парсим вложенные модули
      for (auto node : geometry.Nodes) {
         if (_wcsicmp(node.Name.c_str(), L"module") == 0) {
            mcTransport* eobj = ParseTransport(node, media, nThreads);
            if (eobj == nullptr)
            ...
            static_cast<mcTransportEmbeddedGroup*>(t)->addTransport(eobj);
         }
      }
   } else if (_wcsicmp(geomType.c_str(), L"linear_chain") == 0) {
      t = new mcTransportLinearChain(origin, normal, xaxis); // Ищем и парсим вложенные модули
      for (auto node : geometry.Nodes) {
         if (_wcsicmp(node.Name.c_str(), L"module") == 0) {
            mcTransport* eobj = ParseTransport(node, media, nThreads);
            if (eobj == nullptr)
            ...
            // Корректируем систему координат из группы в сцену.
            eobj->MoveToCoordinateSystem(t->MT2W());
            static_cast<mcTransportLinearChain*>(t)->addTransport(eobj);
         }
      }
      static_cast<mcTransportLinearChain*>(t)->completeInit();
...
{
   mcScore* score = nullptr; // Общие для всех модулей параметры, которые должны быть получены в результате парсинга
   wstring scoreType;
   string scoreModule; // Специфичные для модуля параметры.
   // Для простоты перечисляем все возможные варианты.
   string outfile;
   ...
   bool isxray = false;
   mc_particle_t pt = MCP_PHOTON;
   wstring outdcm;
   wstring calibrdcm; // Структура для накопления данных для последующего создания подисточников
   struct ss_data
   {
      mc_particle_t pt;
      int na;
   ...
   vector<ss_data> subsources; // Контрольные флаги
      ...
      if (_wcsicmp(node.Name.c_str(), L"type") == 0)
         ...
         for (auto n1 : node.Nodes) {
            if (_wcsicmp(n1.Name.c_str(), L"nr") == 0)
               ...
               pt = convert_S2T_ptype(n1.Text.c_str());
         ...
         for (auto n1 : node.Nodes) {
            if (_wcsicmp(n1.Name.c_str(), L"name") == 0)
               ...
               ssrc.pt = convert_S2T_ptype(n1.Text.c_str());
         }
         ...
         pt = convert_S2T_ptype(node.Text.c_str());
      else if (_wcsicmp(node.Name.c_str(), L"particles") == 0)
         pt = static_cast<mc_particle_t>(_wtoi(node.Text.c_str()));
      else if (_wcsicmp(node.Name.c_str(), L"focus") == 0)
         focus = _wtof(node.Text.c_str());
   }
   if (!isSizeSetFound && _wcsicmp(scoreType.c_str(), L"fluence_sphere") != 0 && _wcsicmp(
          scoreType.c_str(), L"fluence_plane") != 0)
   ...
   if (_wcsicmp(scoreType.c_str(), L"phsp") == 0) {
      ...
      for (auto ss : subsources) {
         static_cast<mcScoreBeamFluence*>(score)->addSubsource(new mcScoreBeamFluenceSubsource(
      ...
      score = new mcScoreMatrix2D(scoreModule.c_str(), nThreads, nx, nz, geomVector3D(x1, y1, z1),
                                  geomVector3D(x2, y2, z2));
      ...
      score = new mcScorePhaseSpaceConcentrator(scoreModule.c_str(), nThreads, pt, isxray, focus, ne, nr, nt, na, emax,
                                                rmax, tmax, outfile.c_str());
      ...
      throw exception(
         XmlParseReaderBase::copyWStringToStlString((L"Unsupported score type: " + scoreType).c_str()).c_str());
...
{
   mcSource* source = nullptr; // Общие для всех модулей параметры, которые должны быть получены в результате парсинга
   string srcName;
   string sourceModule;
   wstring radTypeName;
   double energy = 0, ewidth = 0, awidth = 0;
   spectrum_distr_t sptype = SPECTRUM_GAUSS;
   profile_distr_t prf_type = PROFILE_EXPONENT;
   double x0 = 0, y0 = 0, z0 = 0;
   double vx = 0, vy = 0, vz = 0;
   bool isStartInside = false; // Специфичные для модуля параметры.
   // Для простоты перечисляем все возможные варианты.
   wstring fileName;
   double d = 0, h = 0, radius = 0, size = 0, angle = 0;
   double fsx = 0, fsy = 0;
   wstring srcType;
   wstring shapeType; // Контрольные флаги
   ...
   for (auto node : item.Nodes) {
      if (_wcsicmp(node.Name.c_str(), L"name") == 0)
         ...
         isStartInside = _wcsicmp(node.Text.c_str(), L"true") == 0; // Radiation
         ...
         for (auto n1 : node.Nodes) {
            if (_wcsicmp(n1.Name.c_str(), L"type") == 0)
               ...
               sptype = convert_spec_type(n1.Text.c_str());
         ...
         for (auto n1 : node.Nodes) {
            if (_wcsicmp(n1.Name.c_str(), L"direction") == 0)
               ...
               prf_type = convert_profile_distr_type(n1.Text.c_str());
   ...
   if (_wcsicmp(radTypeName.c_str(), L"phsp_photon") == 0) {
      ...
      if (fileName.empty())
         throw exception("Please, indicate phase space model file name"); // Read model data file
      int ifile;
      ...
      static_cast<mcSourceModelRadialPhsp*>(source)->readFromMemory(buf);
      free(buf);
      ...
      source = new mcSourceCylindricalC60(srcName.c_str(), nThreads, geomVector3D(x0, y0, z0), geomVector3D(vx, vy, vz),
                                          d / 2, h);
   } else if (_wcsicmp(radTypeName.c_str(), L"c60_sphere") == 0) {
      if (!isCoreSetDefined)
         throw exception("Please, indicate radiation source dimensions");
      source = new mcSourceSphereC60(srcName.c_str(), nThreads, geomVector3D(x0, y0, z0), geomVector3D(vx, vy, vz),
                                     radius);
   } else if (_wcsicmp(radTypeName.c_str(), L"clinical_electron_beam") == 0) {
      source = new mcClinicalElectronBeam(srcName.c_str(), nThreads, energy, z0, ewidth, awidth, fsx, fsy);
   } else {
      if (!isShapeDefined)
         throw exception("Please, indicate radiation source shape");
      mc_particle_t particleType = convert_S2T_ptype(radTypeName.c_str());
      if (_wcsicmp(srcType.c_str(), L"mono") == 0) {
         if (_wcsicmp(shapeType.c_str(), L"simple") == 0) {
            source = new mcSourceSimpleMono(srcName.c_str(), nThreads, particleType, energy, geomVector3D(x0, y0, z0),
                                            geomVector3D(vx, vy, vz));
         } else if (_wcsicmp(shapeType.c_str(), L"cylindrical") == 0) { } else
