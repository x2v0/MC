...
int _tmain(int argc, _TCHAR* argv [])
{
   ...
   int i;
   int nThreads = concurrency::GetProcessorCount(); //int nThreads = 1;
   ...
   try {
      ...
      media.initXEFromFile("../data/AcceleratorSimulator.pegs4dat"); // Pars input files
      ...
      for (auto node : paramsDoc.Nodes) {
         if (_wcsicmp(node.Name.c_str(), L"simulation") == 0) {
            ...
            mcScore* score = GeometryParser::ParseScore(node, nThreads); // Привязка к геометрическому объекту
      ...
      int nParticles = nHistories / nThreads; // HACK!!!
      ...
      if (ecut > 0) {
         ...
         for (i = 0; i < static_cast<int>(mm.size()); i++) {
            mcMediumXE* m = static_cast<mcMediumXE*>(mm[i]);
            m->transCutoff_elec = ecut; // Test: ручное отключение транспорта электронов в коллиматоре
            //if(m->name_ == "W700ICRU")
            ...
            if (m->name_ == "H2O700ICRU" || m->name_ == "AIR700ICRU" || m->name_ == "LUNG700ICRU" || m->name_ ==
                "ICRPBONE700ICRU")
      ...
      if (!vrmlFile.empty()) {
         ...
         while (t) {
            ...
            if (s)
               s->dumpVRML(wfs); // Проверяем вложения
            mcTransport* ti = t;
            if (ti != nullptr)
      ...
      for (int ib = 0; ib < nBanches; ib++) {
         wcout << L"Time:" << TIMESINCE(simStartTime) << L" sec" << L"       Start bunch " << ib + 1 << L" of " <<
            nBanches << endl;
         if (nThreads == 1) {
            mcParticle particle;
            ...
            concurrency::parallel_for(0u, static_cast<unsigned>(nThreads),
      ...
      wcout << L"Writing results ..." << endl; // Сохранение статистики
      double energyDeposited = 0;
      for (i = 0; i < nThreads; i++)
         energyDeposited += energySource[i];
      wcout << endl;
      ...
      if (!statFile.empty()) {
         ...
         wcout << L"Statistic dump skipped" << endl; // Чистка памяти
      if (source)
         delete source;
      mcTransport* t = tfirst;
      while (t) {
         mcTransport* tt = t;
         t = (mcTransport *)tt->getNextTransport(); // Проверяем вложения
         mcTransport* ti = tt;
         if (ti != nullptr)
            ti = ti->getInternalTransport();
         while (ti) {
